'use strict';

const fs = require('node:fs');
const path = require('node:path');
const test = require('node:test');
const assert = require('node:assert/strict');
const crypto = require('node:crypto');
const { execFileSync } = require('node:child_process');
const programme = require('../scripts/toolkit-github-program-state-v5.cjs');
const governance = require('../scripts/toolkit-github-governance-review-reconciler.cjs');
const receipt = require('../scripts/toolkit-github-program-receipt.cjs');

const repository = programme.REPOSITORY;
const decision = programme.createRecoveryDecision();
const decisionSchemaPath = path.join(__dirname, '..', 'contracts', 'github-program-reconciler', 'projection-bootstrap-recovery-decision-v1.schema.json');
const publishedDecisionSchema = JSON.parse(fs.readFileSync(decisionSchemaPath, 'utf8'));
const finalisationDecisionSchemaPath = path.join(__dirname, '..', 'contracts', 'github-program-reconciler', 'post-merge-epoch-finalisation-decision-v1.schema.json');
const finalisationEvidenceSchemaPath = path.join(__dirname, '..', 'contracts', 'github-program-reconciler', 'post-merge-epoch-finalisation-evidence-v1.schema.json');
const programmeStateSchemaPath = path.join(__dirname, '..', 'contracts', 'github-program-reconciler', 'programme-state-v5.schema.json');
const publishedFinalisationDecisionSchema = JSON.parse(fs.readFileSync(finalisationDecisionSchemaPath, 'utf8'));
const publishedFinalisationEvidenceSchema = JSON.parse(fs.readFileSync(finalisationEvidenceSchemaPath, 'utf8'));
const publishedProgrammeStateSchema = JSON.parse(fs.readFileSync(programmeStateSchemaPath, 'utf8'));
const PARENT_BODY = "<!-- AI-AGENT-TOOLKIT:GITHUB-PROGRAM-PARENT:BEGIN v5 -->\n# Programme dashboard\n\n## Goal\nDeliver the six-stage Toolkit programme through truthful, deterministic, source-traceable programme views.\n\n## Programme status\n| Field | Value |\n| --- | --- |\n| Aggregate programme state | HELD |\n| Concurrency mode | SINGLE_DEFAULT |\n| Active lanes | 1 |\n| Max lanes | 1 |\n| Current child IDs | #359 |\n| Programme finality state | HELD |\n\n## Active work\n| Child | State | Epoch / Gate | Candidate / PR | Material hold |\n| --- | --- | --- | --- | --- |\n| #359 - S2 — Productize retained skills + native host adapters | ACTIVE | E3 / G4 | PR #366 @ a7dcb69da43100c5411076008307a221e89b720f | None |\n\n## Children\n| Child | Lifecycle | Dependencies | Outcome |\n| --- | --- | --- | --- |\n| #358 - S1 — Canonical topology collapse + permanent Ledger retirement | COMPLETED | None | S1 — Canonical topology collapse + permanent Ledger retirement is completed with retained evidence. |\n| #359 - S2 — Productize retained skills + native host adapters | CURRENT | None | S2 — Productize retained skills + native host adapters is current in E3 at G4. |\n| #360 - S3 — Repository Loop Manager + trusted CI live integration | QUEUED | #358 | S3 — Repository Loop Manager + trusted CI live integration is queued behind its declared dependencies. |\n| #361 - S4 — Official n8n Skills + API-first workflow transport | QUEUED | #358, #359, #360 | S4 — Official n8n Skills + API-first workflow transport is queued behind its declared dependencies. |\n| #362 - S5 — External authority, secrets + hosted operations | QUEUED | #358, #359, #360 | S5 — External authority, secrets + hosted operations is queued behind its declared dependencies. |\n| #363 - S6 — Native UAT + final whole-Toolkit assurance | QUEUED | #358, #359, #360, #361, #362 | S6 — Native UAT + final whole-Toolkit assurance is queued behind its declared dependencies. |\n\n## Progress\n| Metric | Value |\n| --- | --- |\n| Completed children / total | 1 / 6 |\n| Retired children | 0 |\n| Accepted or retired epochs / total | 3 / 9 |\n| Active lanes | 1 |\n| Web-decision-required lanes | 0 |\n\n## Major holds\nNone\n\n## Additional context\nNone\n\n<!-- AI-AGENT-TOOLKIT:GITHUB-PROGRAM-CANONICAL v5  -->\n<!-- AI-AGENT-TOOLKIT:GITHUB-PROGRAM-PARENT:END -->";
const CHILD_BODY = "<!-- AI-AGENT-TOOLKIT:GITHUB-PROGRAM-CHILD:BEGIN v5 -->\n# S2 — Productize retained skills + native host adapters\n\n## Summary\nE1 and E2 are accepted; E3 Repair-2 implementation, v5 migration/readback/zero-delta and dogfood UX correction are achieved; fresh G4 is now active with no result.\n\n## Operating contract\n| Field | Value |\n| --- | --- |\n| Parent | #240 |\n| Lane | child-359 |\n| Lifecycle | CURRENT |\n| Epoch | E3 |\n| Gate | G4 |\n| Gate state | ACTIVE |\n| Lock | DL-S2-GITHUB-PROGRAM-CONVERGENCE-002 |\n| Finality | HELD |\n\n## Objective\nProductise retained skills, complete the GitHub programme reconciler, then finish truthful native host adapters.\n\n## Deliverables\n- Retained-skill productisation.\n- GitHub programme reconciler v5.\n- Future E4 truthful native adapters.\n\n## Done when\n- E1 and E2 remain accepted with retained evidence.\n- E3 Repair-2, v5 migration/readback/zero-delta, canonical-data UX correction, fresh G4 and Web acceptance are complete.\n- E4 truthful native adapters are complete and Web records S2 finality.\n\n## Scope\n- Retained-skill productisation, GitHub programme reconciler v5 and future E4 truthful native adapters.\n- Fresh E3 G4 transition and exact-candidate validation.\n\n## Out of scope\n- G4 result or E3 acceptance before separate Web authority.\n- Ready, merge, finality, E4 execution and S3 through S6 progression.\n\n## Progress\n- E1: ACCEPTED\n- E2: ACCEPTED\n- E3: G4 ACTIVE\n- E4: PENDING\n\n## Achieved\n- E1 ACCEPTED\n- E2 ACCEPTED\n- E3 G4 AMEND\n- E3 G2 ACCEPTED\n- E3 G3 ACCEPTED\n\n## Remaining\n- E3 G4\n- E4 G1\n- E4 G2\n- E4 G3\n- E4 G4\n- Web finality disposition\n\n## Epochs / Locks\n| Epoch | Lock | State | Purpose |\n| --- | --- | --- | --- |\n| E1 | DL-S2-CREATION-GATE-003 | ACCEPTED | Creation and admission gate |\n| E2 | DL-S2-PRODUCT-PORTFOLIO-015 | ACCEPTED | Retained product portfolio |\n| E3 | DL-S2-GITHUB-PROGRAM-CONVERGENCE-002 | ACTIVE | Managed GitHub programme reconciliation |\n| E4 | DL-S2-NATIVE-ADAPTERS-002 | PENDING | Truthful native host adapters |\n\n## PR registry\n| PR | Status | Role | Completes Child |\n| --- | --- | --- | --- |\n| #366 | ACTIVE | INTERMEDIATE | No |\n\n## Holds\nNone\n\n## Boundaries\n- Web owns fresh G4 disposition, E3 acceptance, Ready, merge and finality.\n- Fresh G4 is active with no result yet.\n- E4 and S3 through S6 do not begin in this transition.\n\n## Next action\nComplete E3 G4 without advancing finality.\n\n## ELI5\nThe programme tool is repaired and its final independent E3 review gate is now active, but no result or acceptance exists yet.\n\n## Additional context\nNone\n\n<!-- AI-AGENT-TOOLKIT:GITHUB-PROGRAM-PROJECTION v1  -->\n<!-- AI-AGENT-TOOLKIT:GITHUB-PROGRAM-CHILD:END -->";
const PR379_REVIEW_BODY = "## WEB EXACT-HEAD G3 BLOCKED ADJUDICATION / REPAIR-1 AUTHORITY\n\nExact reviewed head: `d3587fb69f0043d74936c116168a17247174ba07` / tree `1cf1bfbccee02b757040350c872bf18ad8056071` against canonical main `c72028c63cc09dd07d3e522692065448b6b7dbb6`.\n\nWeb accepts the G3 implementation structurally but does not accept it as G3-complete because both hosted Validate jobs fail.\n\nThe two failures are adjudicated as same-root test harness/fixture defects only:\n1. real receipt-store tamper test reopens/reads the deliberately corrupted store outside the intended fail-closed assertion boundary;\n2. real SQLite recovery fixture references `harnessA.scope.scope_digest` instead of canonical `harnessA.scope.grant.scope_digest`.\n\nNo runtime, G1 or G2 defect is presently established.\n\nControlling Repair-1 authority: #359 comment `5578743909`.\nParent mirror: #240 comment `5578745835`.\n\nRepair 1 may change only the receipt test, programme-bootstrap-recovery test, then a bootstrap-only repin. No other candidate mutation is authorised. Repair 2, G4, Ready, merge and Programme Apply remain unauthorised pending Web adjudication.";
const PR379_COMMENT_BODIES = Object.freeze({"issue-240-5580975069":"## WEB / S2 E3 RECOVERY G2 ACCEPTED — G3 RECOVERY IMPLEMENTATION CURRENT\n\nRUN=2026-09-08-toolkit-s2-e3-v5-projection-bootstrap-recovery-g2-adjudication\nCHILD=359\nRECOVERY_ROOT=E3-V5-PROGRAMME-PROJECTION-BOOTSTRAP-RECOVERY-001\nLOCK=DL-S2-E3-V5-PROJECTION-BOOTSTRAP-RECOVERY-001\nCONTROLLING_CHILD_AUTHORITY=#359/5580972753\n\nMAIN=c72028c63cc09dd07d3e522692065448b6b7dbb6\nMAIN_TREE=1b7d3fe13251ae91b93498a87248b85c15268c26\nPARENT_RECONCILIATION_INCOMPLETE=YES_PROJECTION_STALE\n\nWeb accepts the recovery G2 contract without another worker rerun. The G2 worker correctly found that GitHub issue PATCH has no documented provider-enforced conditional-write precondition, so the recovery must not claim CAS.\n\nAccepted recovery-only write boundary for the later separately authorised Apply:\n`WRITE_SAFETY_MODE=WEB_EXCLUSIVE_SINGLE_WRITER_RECOVERY_WINDOW`.\n\nDuring that eventual window Web/owner reserves #359/#240 bodies against all authorised competing Toolkit/manual writes; the designated writer performs fresh rebind, serial #359-first/#240-last writes, exact readback and immediate zero delta. The small non-cooperating external GET-to-PATCH race is explicit residual risk accepted only for this two-body control-plane recovery, not described as CAS and not a general product guarantee.\n\nThe G2 nine-path lock is accepted unchanged otherwise. No version bump: Toolkit remains 2.10.8. Source commit S owns eight recovery core paths; bootstrap commit B owns only `.github/ai-agent-toolkit-programme.json`. Receipt runtime/schema, historical-receipt work, v4 migration, managed events, provider clients, workflows, plugin/local-bridge/version churn and all other paths remain forbidden.\n\nRecovery G3 is now authorised from exact canonical main. G3 may create/push a fresh branch and Draft recovery PR and publish only S then B under the accepted scope. It may not mutate #240/#359 bodies, perform Programme Apply, mutate #379, launch normal E3, run G4, mark Ready, merge or advance finality.\n\nG3_RECOVERY_IMPLEMENTATION_AUTHORISED=YES\nPROGRAMME_APPLY_AUTHORISED=NO\nG4_AUTHORISED=NO_PENDING_EXACT_HEAD_WEB_ADJUDICATION\nREADY_AUTHORISED=NO\nMERGE_AUTHORISED=NO\nNORMAL_PROGRAMME_PROGRESS_AUTHORISED=NO\n\nPR #379 remains OPEN/DRAFT/UNMERGED/FROZEN. Old E3-003 remains terminal non-convergent at 2/2 with no Repair 3. Parked root `E3-HISTORICAL-RECEIPT-CI-PROOF-BOUNDARY-SIMPLIFICATION-004` remains NOT_LAUNCHED.\n\nNEXT_ACTION=Recovery-only G3 under #359 controlling authority.\nSECRET_CLASSIFICATION=none\n\nRefs #359\nRefs #379","issue-240-5580534575":"## WEB / S2 E3 RECOVERY G1 ACCEPTED — MINIMISED G2 CURRENT\n\nRUN=2026-09-08-toolkit-s2-e3-v5-projection-bootstrap-g1-adjudication\nCHILD=359\nRECOVERY_ROOT=E3-V5-PROGRAMME-PROJECTION-BOOTSTRAP-RECOVERY-001\nG1_RESULT=ACCEPTED_WITH_WEB_SCOPE_REFINEMENT\nG2_LOCK=DL-S2-E3-V5-PROJECTION-BOOTSTRAP-RECOVERY-001\nG2_CURRENT=YES_READ_ONLY\n\nMAIN=c72028c63cc09dd07d3e522692065448b6b7dbb6\nMAIN_TREE=1b7d3fe13251ae91b93498a87248b85c15268c26\nPR379_HEAD=adca2ffec8322eb57afcd9f9fdc67210503ebcf1\nPR379_TREE=2c712aa9c7c6e66a89bdd9d033acece5415fd575\nPARENT_RECONCILIATION_INCOMPLETE=YES_PROJECTION_STALE\n\nParent mirror of the controlling Child #359 Web G1 adjudication. Web accepts Option B: a fresh minimal recovery bootstrap from canonical main is the correct architecture. The live/candidate evidence confirms three real v5 expressivity gaps: held CURRENT cannot currently have zero normal active lanes, the PR registry lacks RETAINED, and historical state lacks typed NON_CONVERGENT/repair-budget semantics.\n\nScope refinement is binding: the G1 worker's 32-path proposal is not a G3 allowlist. Recovery G2 must minimise the source surface and justify every allowed path. Package/plugin/local-bridge/version-alignment and broad docs/audit churn are presumptively excluded unless a canonical rule proves them strictly necessary. The #379 v5 surface contract must not be copied wholesale because historical-receipt `readReceiptById` and v4 migration remain outside this recovery. No receipt runtime/schema change, managed-event change, provider client, workflow/fetch-depth change, or caller-supplied full desired state is allowed.\n\nThe future mechanism must render only the deterministic #240/#359 recovery state, bind complete evidence, plan exactly #359-first/#240-last body operations, fail closed on movement/partial writes, perform exact readback + zero delta, use the existing receipt subsystem unchanged, and—if implementation is later authorised—publish source S then bootstrap-only B. Any later merge must preserve S; merge-commit is available in repository settings.\n\nPR #379 remains frozen OPEN/DRAFT/UNMERGED. Old E3-003 remains terminal non-convergent at repair budget 2/2 with no Repair 3. Parked root `E3-HISTORICAL-RECEIPT-CI-PROOF-BOUNDARY-SIMPLIFICATION-004` remains NOT_LAUNCHED.\n\nG3_RECOVERY_IMPLEMENTATION_AUTHORISED=NO_PENDING_G2\nPARKED_ROOT_G2_AUTHORISED=NO\nPROGRAMME_APPLY_AUTHORISED=NO\nG4_AUTHORISED=NO\nREADY_AUTHORISED=NO\nMERGE_AUTHORISED=NO\nNORMAL_PROGRAMME_PROGRESS_AUTHORISED=NO\n\nNEXT_ACTION=Fresh read-only recovery G2 Design Lock, then Web adjudication.\nSECRET_CLASSIFICATION=none\n\nRefs #359\nRefs #379","issue-359-5580972753":"## WEB / RECOVERY G2 CAS ADJUDICATION — G2 ACCEPTED WITH SINGLE-WRITER AMENDMENT; G3 AUTHORISED\n\nRUN=2026-09-08-toolkit-s2-e3-v5-projection-bootstrap-recovery-g2-adjudication\nRECOVERY_ROOT=E3-V5-PROGRAMME-PROJECTION-BOOTSTRAP-RECOVERY-001\nLOCK=DL-S2-E3-V5-PROJECTION-BOOTSTRAP-RECOVERY-001\nG2_WORKER_RESULT=GATE_REENTRY_REQUIRED\nWEB_G2_DISPOSITION=ACCEPTED_WITH_EXPLICIT_WRITE_TRUST_BOUNDARY_AMENDMENT\nG2_RERUN_REQUIRED=NO\n\nMAIN=c72028c63cc09dd07d3e522692065448b6b7dbb6\nMAIN_TREE=1b7d3fe13251ae91b93498a87248b85c15268c26\nPR379_HEAD=adca2ffec8322eb57afcd9f9fdc67210503ebcf1\nPR379_STATE=OPEN_DRAFT_UNMERGED_FROZEN\nPARENT_RECONCILIATION_INCOMPLETE=YES_PROJECTION_STALE\n\nWeb independently confirms the worker's narrow blocker: GitHub REST conditional requests are generally unsupported for unsafe methods unless a specific endpoint documents otherwise, and the issue-body PATCH endpoint exposes no ETag/expected-revision write precondition. Therefore prewrite rebind + PATCH + readback MUST NOT be called provider CAS.\n\nWeb resolves this as a recovery-only trust-boundary decision rather than rerunning G2:\n\nWRITE_SAFETY_MODE=WEB_EXCLUSIVE_SINGLE_WRITER_RECOVERY_WINDOW\nPROVIDER_CAS_CLAIM=FORBIDDEN\nRESIDUAL_EXTERNAL_RACE=EXPLICITLY_ACCEPTED_FOR_THIS_RECOVERY_ONLY\n\nThe eventual Programme Apply window, if separately authorised after merge/preview, is valid only while all of these hold:\n1. Web/owner reserves issue bodies #359 and #240 exclusively to the single designated recovery writer for the duration of the two ordered body writes and readback.\n2. No other authorised Toolkit worker, controller lane, automation, manual owner edit, or reconciliation action may mutate #359/#240 bodies during that window.\n3. Writes are strictly serial: fresh complete evidence/rebind -> #359 write -> exact #359 readback -> fresh complete rebind of all external facts -> #240 write -> full readback -> immediate zero delta.\n4. Any observed movement, authority change, body difference, label/native relationship movement, PR movement, partial-state mismatch, or inability to maintain the reservation fails closed to Web.\n5. The existing A1/receipt machinery binds operation identity and serialisation but MUST NOT be represented as provider CAS. Do not use a receipt/operation label that asserts CAS or conditional-provider enforcement when none exists. Bind the accepted `WEB_EXCLUSIVE_SINGLE_WRITER_RECOVERY_WINDOW` safety mode in the recovery decision/preview/operation digest using existing generic receipt admission semantics.\n6. The residual risk is the narrow non-cooperating external GET-to-PATCH race which GitHub cannot atomically exclude. Web accepts that residual risk only because this is a one-time two-body governance-projection recovery with exact deterministic target bytes, no repository/source/PR/label/native/finality mutation, serial operation, and full immediate readback. This is NOT a general claim that GitHub issue-body writes are race-free and creates no precedent to describe ordinary issue PATCH as CAS.\n7. If G3 discovers the unchanged canonical receipt API cannot truthfully bind this operation without modifying receipt source/schema or falsely labelling it CAS, stop with `GATE_REENTRY_REQUIRED`; receipt changes remain outside scope.\n\nThe remainder of the G2 contract is accepted. The final implementation scope is exactly nine paths, with no version bump:\n\nSOURCE COMMIT S — exactly eight paths:\n- repo/contracts/github-program-reconciler/controller-bootstrap-v1.schema.json\n- repo/contracts/github-program-reconciler/programme-state-v5.schema.json\n- repo/contracts/github-program-reconciler/programme-surface-contract-v5.json\n- repo/contracts/github-program-reconciler/projection-bootstrap-recovery-decision-v1.schema.json\n- repo/contracts/github-program-reconciler/projection-bootstrap-recovery-evidence-v1.schema.json\n- repo/scripts/toolkit-github-program-state-v5.cjs\n- repo/scripts/toolkit-github-governance-review-reconciler.cjs\n- repo/tests/toolkit-github-programme-projection-bootstrap-recovery-v5.test.cjs\n\nBOOTSTRAP COMMIT B — exactly one path:\n- .github/ai-agent-toolkit-programme.json\n\nALLOWED_PATH_COUNT=9\nVERSION_BUMP_REQUIRED=NO\nTOOLKIT_VERSION_REMAINS=2.10.8\n\nAll other paths are forbidden, including receipt runtime/schema, historical `readReceiptById`, v4/migration/managed-event surfaces, plugin/local-bridge/version alignment, skill/docs/audit churn, provider client code and `.github/workflows/**`.\n\nG3 publication contract:\n- fresh branch from exact main c72028c63cc09dd07d3e522692065448b6b7dbb6;\n- S direct child of main, exactly the eight source/contract/test paths;\n- B direct child of S, bootstrap-only;\n- hosted bootstrap proof uses checked-in bootstrap + workspace contract bytes, never parent Git-object lookup;\n- create/retain a Draft recovery PR;\n- run focused tests, existing affected receipt/reconciler suites, validate-toolkit, git diff --check and applicable audits;\n- hosted Validate, Validate toolkit and CodeQL/language analyses must be green before G4 admission;\n- no mutation of #240/#359 bodies during G3;\n- no Programme Apply, G4, Ready, merge or finality in G3.\n\nG3_RECOVERY_IMPLEMENTATION_AUTHORISED=YES\nPROGRAMME_APPLY_AUTHORISED=NO\nG4_AUTHORISED=NO_PENDING_EXACT_HEAD_WEB_ADJUDICATION\nREADY_AUTHORISED=NO\nMERGE_AUTHORISED=NO\nPARKED_ROOT_G2_AUTHORISED=NO\nNORMAL_PROGRAMME_PROGRESS_AUTHORISED=NO\n\nOLD_ROOT=E3-CANONICAL-HISTORICAL-RECEIPT-RESOLUTION-003\nOLD_ROOT_DISPOSITION=TERMINAL_NON_CONVERGENT\nREPAIR_BUDGET=2/2_CONSUMED\nREPAIR3_AUTHORISED=NO\n\nPARKED_ROOT=E3-HISTORICAL-RECEIPT-CI-PROOF-BOUNDARY-SIMPLIFICATION-004\nPARKED_ROOT_STATUS=NOT_LAUNCHED\n\nNEXT_ACTION=Run recovery-only G3 on the exact nine-path lock and return exact S/B/PR/validation evidence to Web.\nSECRET_CLASSIFICATION=none\n\nRefs #240\nRefs #379","issue-359-5580530088":"## WEB / RECOVERY G1 ACCEPTED — MINIMISED G2 DESIGN LOCK CURRENT\n\nRUN=2026-09-08-toolkit-s2-e3-v5-projection-bootstrap-g1-adjudication\nRECOVERY_ROOT=E3-V5-PROGRAMME-PROJECTION-BOOTSTRAP-RECOVERY-001\nG1_RESULT=ACCEPTED_WITH_WEB_SCOPE_REFINEMENT\nG2_LOCK=DL-S2-E3-V5-PROJECTION-BOOTSTRAP-RECOVERY-001\nG2_CURRENT=YES_READ_ONLY\n\nMAIN=c72028c63cc09dd07d3e522692065448b6b7dbb6\nMAIN_TREE=1b7d3fe13251ae91b93498a87248b85c15268c26\nPR379_HEAD=adca2ffec8322eb57afcd9f9fdc67210503ebcf1\nPR379_TREE=2c712aa9c7c6e66a89bdd9d033acece5415fd575\nPR379_DISPOSITION=RETAIN_OPEN_DRAFT_UNMERGED_FROZEN\nPARENT_RECONCILIATION_INCOMPLETE=YES_PROJECTION_STALE\n\nWeb accepts the G1 architectural conclusion: use a fresh minimal recovery bootstrap PR from canonical main. Existing trusted v3/A1/A2/receipt primitives are reusable, but cannot alone parse/render the live v5 state or express the required held/retained/non-convergent semantics. A one-off external renderer and wholesale #379 reuse remain rejected.\n\nWeb independently confirmed the material candidate-v5 gaps: CURRENT children are currently forced to have an active lane; PR registry states do not include RETAINED; historical transition dispositions do not include NON_CONVERGENT. These are real schema/runtime expressivity gaps, not merely stale Markdown.\n\n### Web scope refinement\n\nThe G1 packet's proposed 32-path implementation allowlist is NOT accepted as the G3 allowlist. G2 must derive the smallest exact recovery-core path set and justify every path.\n\nPresumptively OUT unless G2 proves an existing canonical validation/publication rule makes a change strictly unavoidable:\n- package/plugin/local-bridge version bump or alignment churn;\n- `.claude-plugin/**`, `.codex-plugin/**`, local-bridge version surfaces, agent-control/setup/delegation version constants;\n- published-surface/Safety Matrix/Skill Creation baseline churn;\n- broad skill documentation rewrites;\n- `validate-toolkit.cjs` changes beyond demonstrably necessary contract registration.\n\nThe #379 `programme-surface-contract-v5.json` must NOT be reused wholesale: it includes parked historical-receipt and migration semantics that are outside this recovery. Recovery G2 must exclude `readReceiptById`, historical-receipt resolution, v4 migration, managed-event changes, receipt schema/runtime changes or duplication, provider-client code, and `.github/workflows/**`/fetch-depth changes.\n\nG2 must lock a closed, typed recovery decision/evidence boundary for this exact stale state rather than accepting a caller-authored complete `desired` state. The minimum semantic target remains: #359 CURRENT + blocking recovery hold + zero normal active lanes; #366 RETIRED/CLOSED_UNMERGED; #379 RETAINED/OPEN_DRAFT/INTERMEDIATE/completes_child=false with exact candidate binding; old E3-003 NON_CONVERGENT with 2/2 consumed and further repair forbidden; no G4 active; E3 unaccepted; E4/S3-S6 blocked; parked proof-boundary root not launched.\n\nG2 must also lock:\n- deterministic #240/#359 rendering and digests only;\n- complete typed Web/GitHub/native evidence and movement rejection;\n- exactly two eventual Programme Apply body operations: #359 first, #240 parent last;\n- CAS/prewrite rebinding, partial-write fail-closed recovery, full readback and immediate zero delta;\n- existing receipt subsystem unchanged for the new recovery operation only;\n- source commit S followed by bootstrap-only commit B if implementation is later authorised;\n- hosted bootstrap/contract proof from checked-in workspace bytes, never historical parent Git-object lookup or widened checkout history;\n- separate exact S:path/digest provenance proof through full GitHub/G4 evidence;\n- merge-commit preservation of S if Web later accepts G4 and merge. Repository settings currently permit merge commits.\n\nG2 is READ-ONLY. It may inspect #379 only as non-authoritative design evidence. It must return an exact file allowlist/forbidden list, necessity for each allowed path, schema shapes, version-bump decision with evidence, commit plan, tests, hosted checks, G4 scope, merge method, Apply/CAS/readback contract and retirement rule.\n\nOLD_ROOT=E3-CANONICAL-HISTORICAL-RECEIPT-RESOLUTION-003\nOLD_ROOT_DISPOSITION=TERMINAL_NON_CONVERGENT\nREPAIR_BUDGET=2/2_CONSUMED\nREPAIR3_AUTHORISED=NO\n\nPARKED_ROOT=E3-HISTORICAL-RECEIPT-CI-PROOF-BOUNDARY-SIMPLIFICATION-004\nPARKED_ROOT_STATUS=NOT_LAUNCHED\n\nG3_RECOVERY_IMPLEMENTATION_AUTHORISED=NO_PENDING_G2\nPARKED_ROOT_G2_AUTHORISED=NO\nPROGRAMME_APPLY_AUTHORISED=NO\nG4_AUTHORISED=NO\nREADY_AUTHORISED=NO\nMERGE_AUTHORISED=NO\nNORMAL_PROGRAMME_PROGRESS_AUTHORISED=NO\n\nNEXT_ACTION=Run fresh recovery-only G2 Design Lock and return to Web.\nSECRET_CLASSIFICATION=none\n\nRefs #240\nRefs #379","5580538176":"## WEB / PR #379 REMAINS FROZEN — RECOVERY G2 CURRENT\n\nRUN=2026-09-08-toolkit-s2-e3-v5-projection-bootstrap-g1-adjudication\nRECOVERY_ROOT=E3-V5-PROGRAMME-PROJECTION-BOOTSTRAP-RECOVERY-001\nG2_LOCK=DL-S2-E3-V5-PROJECTION-BOOTSTRAP-RECOVERY-001\nG2_CURRENT=YES_READ_ONLY\n\nEXACT_HEAD=adca2ffec8322eb57afcd9f9fdc67210503ebcf1\nEXACT_TREE=2c712aa9c7c6e66a89bdd9d033acece5415fd575\nMAIN=c72028c63cc09dd07d3e522692065448b6b7dbb6\nPR_DISPOSITION=RETAIN_OPEN_DRAFT_UNMERGED_FROZEN\n\nWeb accepted the recovery G1 architecture with a scope refinement: a fresh minimal v5 projection-bootstrap PR from canonical main is preferred, but the G1 worker's proposed 32-file implementation footprint is not accepted as the G3 allowlist.\n\nThis PR remains non-authoritative design evidence only. G2 may inspect its v5 parser/renderer/schema ideas, but must treat reused fragments as new source and must not copy the candidate v5 surface contract wholesale. In particular, this recovery excludes this PR's historical-receipt `readReceiptById` work, v4 migration work, receipt runtime/schema changes, managed-event changes, provider/client work, workflow/fetch-depth changes, and broad package/plugin/local-bridge/version churn unless independently proven necessary.\n\nThe old root remains terminal non-convergent at 2/2 repairs and no Repair 3. No mutation of this branch, PR body/state, checks, reviews, or commits is authorised by recovery G2.\n\nPARENT_RECONCILIATION_INCOMPLETE=YES_PROJECTION_STALE\nREPAIR_BUDGET=2/2_CONSUMED\nREPAIR3_AUTHORISED=NO\nPARKED_ROOT=E3-HISTORICAL-RECEIPT-CI-PROOF-BOUNDARY-SIMPLIFICATION-004\nPARKED_ROOT_STATUS=NOT_LAUNCHED\n\nG3_RECOVERY_IMPLEMENTATION_AUTHORISED=NO_PENDING_G2\nPARKED_ROOT_G2_AUTHORISED=NO\nPROGRAMME_APPLY_AUTHORISED=NO\nG4_AUTHORISED=NO\nREADY_AUTHORISED=NO\nMERGE_AUTHORISED=NO\n\nSECRET_CLASSIFICATION=none\n\nRefs #240\nRefs #359","5580978455":"## WEB / PR #379 STILL FROZEN — SEPARATE RECOVERY G3 AUTHORISED\n\nRUN=2026-09-08-toolkit-s2-e3-v5-projection-bootstrap-recovery-g2-adjudication\nRECOVERY_ROOT=E3-V5-PROGRAMME-PROJECTION-BOOTSTRAP-RECOVERY-001\nCONTROLLING_CHILD_AUTHORITY=#359/5580972753\n\nEXACT_HEAD=adca2ffec8322eb57afcd9f9fdc67210503ebcf1\nEXACT_TREE=2c712aa9c7c6e66a89bdd9d033acece5415fd575\nMAIN=c72028c63cc09dd07d3e522692065448b6b7dbb6\nPR_DISPOSITION=RETAIN_OPEN_DRAFT_UNMERGED_FROZEN\n\nRecovery G2 is accepted with a Web-only write-safety amendment. GitHub issue PATCH is not provider CAS; later Apply, if separately authorised, will use the explicitly bounded `WEB_EXCLUSIVE_SINGLE_WRITER_RECOVERY_WINDOW` with residual external race disclosed. No Apply is authorised now.\n\nA separate fresh recovery G3 is authorised from canonical main using exactly the accepted nine-path recovery lock, with no version bump and source S + bootstrap-only B commits. That G3 is NOT a mutation or Repair 3 of this PR.\n\nThis PR remains non-authoritative design evidence only. Do not mutate its branch, commits, body, state, checks or reviews. Do not import its historical-receipt `readReceiptById` or migration work into the recovery core.\n\nREPAIR_BUDGET=2/2_CONSUMED\nREPAIR3_AUTHORISED=NO\nPROGRAMME_APPLY_AUTHORISED=NO\nG4_AUTHORISED=NO\nREADY_AUTHORISED=NO\nMERGE_AUTHORISED=NO\nPARENT_RECONCILIATION_INCOMPLETE=YES_PROJECTION_STALE\n\nPARKED_ROOT=E3-HISTORICAL-RECEIPT-CI-PROOF-BOUNDARY-SIMPLIFICATION-004\nPARKED_ROOT_STATUS=NOT_LAUNCHED\n\nSECRET_CLASSIFICATION=none\n\nRefs #240\nRefs #359","5579264600":"## WEB REPAIR-1 BLOCKED ADJUDICATION / FINAL REPAIR-2 LINK\n\nAUTHORITY_RECORD=2026-09-08-web-e3-003-final-repair2\nROOT=E3-CANONICAL-HISTORICAL-RECEIPT-RESOLUTION-003\n\nControlling authority: [Child #359 comment 5579252134](https://github.com/weijunswj/ai-agent-toolkit/issues/359#issuecomment-5579252134).\nVerified parent mirror: [#240 comment 5579259452](https://github.com/weijunswj/ai-agent-toolkit/issues/240#issuecomment-5579259452).\n\nThis adjudication binds Repair-1 head fd8413b75d9977b6a9c3d9d68c92c48c834a7e81 / tree 6deec8ef47b34152ec18129a0885d4ef2dc1e91d against main c72028c63cc09dd07d3e522692065448b6b7dbb6.\n\nRepair 1 is not G3-complete. Its original two defects are closed, but both Validate checks remain failed. Web reproduced the recovery assertion locally and also found a bootstrap-resolution defect:\n- Programme recovery requires TERMINAL_UNCONSUMED, with replay_allowed=false and advances_state=false; TERMINAL is the lower receipt-store status.\n- The actual bootstrap contains raw hash d65cd2ad008998ed79921c21c2d55195eabbd63b70cc098336966120bb33266a, whereas the existing resolver requires canonical-JSON hash 50c1b3eb438ac22e3da52367ae08f76d5abc622e8c8da75312b4f36673f3c494. Exact pinned resolution fails until the bootstrap value is corrected. The PR description's earlier hash claim is not evidence of resolver acceptance.\n\nFinal Repair 2 may change only the programme-bootstrap-recovery test and then the bootstrap revision/digest, as fully specified in the child authority. Required append-only sequence: current head -> R2-SOURCE (test only) -> R2-BOOTSTRAP (bootstrap only). No runtime, schema, contract, policy, workflow, version or other file changes.\n\nBudget remains 1/2 consumed until the first Repair-2 source mutation, then 2/2. No Repair 3. Final repaired-head validation and fresh Web adjudication are required before any G4.\n\nPR remains OPEN/DRAFT/INTERMEDIATE; completes_child=false.\nG4_AUTHORISED=NO_PENDING_WEB_ADJUDICATION\nREADY_AUTHORISED=NO\nMERGE_AUTHORISED=NO\nPROGRAMME_APPLY_AUTHORISED=NO\nPARENT_RECONCILIATION_INCOMPLETE=YES_RECOVERY_IN_PROGRESS\nSECRET_CLASSIFICATION=none\n\nNo worker was launched and no candidate or managed programme state was changed by this publication.","5579508129":"## G3 FINAL REPAIR-2 EVIDENCE / PARENT MIRROR\n\nParent mirror: https://github.com/weijunswj/ai-agent-toolkit/issues/240#issuecomment-5579503003\n\nR2-BOOTSTRAP exact head: adca2ffec8322eb57afcd9f9fdc67210503ebcf1\nR2-SOURCE: 0a0990d7dd2e48c8522a6dfd20dc60a28b33f8ed\n\nHosted Validate and Validate toolkit both fail only at the new actual-bootstrap regression: the test asks a shallow checkout for the pinned R2-SOURCE Git object and receives status 128. The checked-in resolver, canonical digest proof, receipt lookup/tamper coverage, retained-v3 historical resolution, and all four analysis/security contexts otherwise pass. Repair budget is 2/2; no Repair 3 is authorised.\n\nRESULT=NON_CONVERGENCE_DECISION_REQUIRED\nG4_AUTHORISED=NO_PENDING_WEB_ADJUDICATION\nREADY_AUTHORISED=NO\nMERGE_AUTHORISED=NO\nPROGRAMME_APPLY_AUTHORISED=NO","5579738186":"## WEB / PR #379 NON-CONVERGENCE DISPOSITION\n\nRUN=2026-09-08-toolkit-s2-e3-non-convergence-adjudication\nOLD_ROOT=E3-CANONICAL-HISTORICAL-RECEIPT-RESOLUTION-003\nOLD_ROOT_DISPOSITION=TERMINAL_NON_CONVERGENT\nREPAIR_BUDGET=2/2_CONSUMED\nREPAIR3_AUTHORISED=NO\nEXACT_HEAD=adca2ffec8322eb57afcd9f9fdc67210503ebcf1\nMAIN=c72028c63cc09dd07d3e522692065448b6b7dbb6\n\nWEB_FINDING=The remaining hosted failure is confined to the Repair-2 actual-bootstrap regression invoking git cat-file/git show for a pinned parent revision unavailable in the hosted shallow checkout. Do not patch this as Repair 3 and do not change fetch depth merely to satisfy the regression.\n\nWEB_DECISION=Retain this PR OPEN/DRAFT/UNMERGED at the exact head. Preserve the 48-path candidate and its existing runtime/schema/receipt semantics.\n\nNEXT_ROOT_INTENT=E3-HISTORICAL-RECEIPT-CI-PROOF-BOUNDARY-SIMPLIFICATION-004\nNEXT_GATE_INTENT=G2_REENTRY\nG2_PURPOSE=Separate hosted workspace-byte resolver proof from full-history revision/object proof.\nG2_LAUNCH_AUTHORISED=NO_PENDING_PROGRAMME_RECONCILIATION\n\nPARENT_RECONCILIATION_INCOMPLETE=YES_PROJECTION_STALE\nREASON=#240 and #359 managed v5 bodies still project PR #366 / E3 G4 ACTIVE and must be reconciled against the current durable #379 chronology before a material gate transition.\n\nNo repository/code mutation, Repair 3, G2 implementation, G4, Ready, merge, finality or Programme Apply is authorised by this comment. Next action is a reconciliation-only control-plane pass and Web adjudication.\n\nSECRET_CLASSIFICATION=none\n\nRefs #240\nRefs #359","5579993168":"## WEB / PR #379 FROZEN DURING V5 PROJECTION BOOTSTRAP RECOVERY\n\nRUN=2026-09-08-toolkit-s2-e3-v5-projection-bootstrap-adjudication\nRECON_RESULT=ROUTE_UNAVAILABLE\n\nEXACT_HEAD=adca2ffec8322eb57afcd9f9fdc67210503ebcf1\nEXACT_TREE=2c712aa9c7c6e66a89bdd9d033acece5415fd575\nMAIN=c72028c63cc09dd07d3e522692065448b6b7dbb6\n\nWEB_CLASSIFICATION=The new blocker is a control-plane bootstrap gap outside this candidate's repair budget: current main lacks a trusted v5 live projection resolver/renderer and authoritative target-state mapping, while this candidate cannot certify or apply its own v5 projection authority.\n\nPR_DISPOSITION=RETAIN_OPEN_DRAFT_UNMERGED_FROZEN\nREPAIR_BUDGET=2/2_CONSUMED\nREPAIR3_AUTHORISED=NO\n\nPARKED_NEXT_ROOT=E3-HISTORICAL-RECEIPT-CI-PROOF-BOUNDARY-SIMPLIFICATION-004\nPARKED_NEXT_GATE=G2_REENTRY\n\nRECOVERY_ROOT=E3-V5-PROGRAMME-PROJECTION-BOOTSTRAP-RECOVERY-001\nRECOVERY_GATE=G1_READ_ONLY_ARCHITECTURE\nRECOVERY_G1_AUTHORISED=YES\n\nThe recovery review may inspect this PR's v5 implementation only as non-authoritative design evidence. It may not mutate this branch, rerun the exhausted same-root repair cycle, use this candidate to self-certify a managed projection, or perform Programme Apply.\n\nNo code mutation, Repair 3, G2 implementation, G4, Ready, merge, finality, or managed-body mutation is authorised here.\n\nSECRET_CLASSIFICATION=none\n\nRefs #240\nRefs #359"});

function digestText(value) { return crypto.createHash('sha256').update(value, 'utf8').digest('hex'); }
function clone(value) { return JSON.parse(JSON.stringify(value)); }
function withEvidenceDigest(value) {
  const next = clone(value);
  delete next.evidence_digest;
  return { ...value, evidence_digest: programme.digestValue(next) };
}
function finalisationAuthority() {
  const body = 'synthetic future Web finalisation write authority for test fixtures';
  return {
    reference: 'synthetic:web-finalisation-authority:001',
    body_digest: digestText(body),
    body,
    repository,
    root: programme.FINALISATION_ROOT,
    lock: programme.FINALISATION_LOCK,
    scope: programme.FINALISATION_SCOPE,
    complete: true,
  };
}
function finalisationDecision() {
  return finalisationDecisionWithRevisions().decision;
}
function finalisationStateFixtures() {
  const source = parsedSources();
  const sourceState = programme.buildRecoveryTargetState(source.parent.state);
  assert.ok(sourceState);
  const stageA = programme.postMergeEpochFinalisation.buildStageATargetState(sourceState);
  assert.ok(stageA);
  const stageB = programme.postMergeEpochFinalisation.buildStageBTargetState(stageA);
  assert.ok(stageB);
  const renderedSource = programme.renderProgrammeV5(sourceState);
  const renderedStageA = programme.renderProgrammeV5(stageA);
  const renderedStageB = programme.renderProgrammeV5(stageB);
  assert.equal(renderedSource.ok, true, renderedSource.code);
  assert.equal(renderedStageA.ok, true, renderedStageA.code);
  assert.equal(renderedStageB.ok, true, renderedStageB.code);
  return {
    sourceState,
    stageA,
    stageB,
    renderedSource,
    renderedStageA,
    renderedStageB,
    sourceParent: programme.parseParentV5Body(renderedSource.parent, { repository, parent_issue: 240 }),
    sourceChild: programme.parseChildV5Body(renderedSource.child, { repository, parent_issue: 240 }),
    stageAParent: programme.parseParentV5Body(renderedStageA.parent, { repository, parent_issue: 240 }),
    stageAChild: programme.parseChildV5Body(renderedStageA.child, { repository, parent_issue: 240 }),
    stageBParent: programme.parseParentV5Body(renderedStageB.parent, { repository, parent_issue: 240 }),
    stageBChild: programme.parseChildV5Body(renderedStageB.child, { repository, parent_issue: 240 }),
  };
}
const FINALISATION_NORMAL_SPECS = Object.freeze({
  BEFORE_STAGE_A: { previous: null, completed: [] },
  CHILD_STAGE_A_OBSERVED: { previous: 'BEFORE_STAGE_A', completed: [1] },
  PARENT_STAGE_A_OBSERVED: { previous: 'CHILD_STAGE_A_OBSERVED', completed: [1, 2] },
  PR379_CLOSED_STAGE_A: { previous: 'PARENT_STAGE_A_OBSERVED', completed: [1, 2, 3] },
  CHILD_STAGE_B_OBSERVED: { previous: 'PR379_CLOSED_STAGE_A', completed: [1, 2, 3, 4] },
  FINAL_TARGET_OBSERVED: { previous: 'CHILD_STAGE_B_OBSERVED', completed: [1, 2, 3, 4, 5] },
});
const FINALISATION_ACK_SPECS = Object.freeze({
  CHILD_STAGE_A_ACKNOWLEDGEMENT_LOST: { pre: 'BEFORE_STAGE_A', base: 'CHILD_STAGE_A_OBSERVED', order: 1, completed: [1] },
  PARENT_STAGE_A_ACKNOWLEDGEMENT_LOST: { pre: 'CHILD_STAGE_A_OBSERVED', base: 'PARENT_STAGE_A_OBSERVED', order: 2, completed: [1, 2] },
  PR379_CLOSE_ACKNOWLEDGEMENT_LOST: { pre: 'PARENT_STAGE_A_OBSERVED', base: 'PR379_CLOSED_STAGE_A', order: 3, completed: [1, 2] },
  CHILD_STAGE_B_ACKNOWLEDGEMENT_LOST: { pre: 'PR379_CLOSED_STAGE_A', base: 'CHILD_STAGE_B_OBSERVED', order: 4, completed: [1, 2, 3, 4] },
  PARENT_STAGE_B_ACKNOWLEDGEMENT_LOST: { pre: 'CHILD_STAGE_B_OBSERVED', base: 'FINAL_TARGET_OBSERVED', order: 5, completed: [1, 2, 3, 4, 5] },
});
function finalisationRevisionSet(overrides = {}) {
  const source = {
    parent: 'synthetic:parent:source',
    child: 'synthetic:child:source',
    pr379: 'synthetic:pr379:source',
    pr380: 'synthetic:pr380:source',
    ...overrides,
  };
  return {
    BEFORE_STAGE_A: source,
    CHILD_STAGE_A_OBSERVED: { ...source, child: 'synthetic:child:stage-a' },
    PARENT_STAGE_A_OBSERVED: { ...source, parent: 'synthetic:parent:stage-a', child: 'synthetic:child:stage-a' },
    PR379_CLOSED_STAGE_A: { ...source, parent: 'synthetic:parent:stage-a', child: 'synthetic:child:stage-a', pr379: 'synthetic:pr379:closed' },
    CHILD_STAGE_B_OBSERVED: { parent: 'synthetic:parent:stage-a', child: 'synthetic:child:stage-b', pr379: 'synthetic:pr379:closed', pr380: source.pr380 },
    FINAL_TARGET_OBSERVED: { parent: 'synthetic:parent:stage-b', child: 'synthetic:child:stage-b', pr379: 'synthetic:pr379:closed', pr380: source.pr380 },
  };
}
function finalisationPr379Facts(githubState = 'OPEN') {
  return {
    repository,
    pr: 379,
    state: githubState,
    github_state: githubState,
    status: 'RETIRED',
    draft: true,
    merged: false,
    merged_at: null,
    head: programme.FROZEN_HEAD,
    tree: programme.FROZEN_TREE,
    branch: programme.FROZEN_BRANCH,
    base_ref: 'main',
    base_sha: programme.MAIN_SHA,
    changed_files: 48,
    candidate: {
      repository,
      branch: programme.FROZEN_BRANCH,
      base_ref: 'main',
      base_sha: programme.MAIN_SHA,
      head: programme.FROZEN_HEAD,
      tree: programme.FROZEN_TREE,
      version: '2.11.0',
    },
    retention_evidence_ref: 'web-pr379-retained-5580538176',
    retirement_evidence_ref: programme.POST_MERGE_TECHNICAL_EVIDENCE_REF,
    non_convergence_evidence_ref: programme.PR379_NON_CONVERGENCE_EVIDENCE_REF,
  };
}
function finalisationPr380Facts() {
  return {
    repository,
    pr: 380,
    state: 'MERGED',
    github_state: 'MERGED',
    status: 'ACCEPTED',
    draft: false,
    merged: true,
    merge_method: 'MERGE_COMMIT',
    merge_commit: programme.MERGE_COMMIT_SHA,
    ordered_parents: [programme.PR380_BASE_SHA, programme.PR380_HEAD],
    accepted_head_tree: programme.PR380_TREE,
    head: programme.PR380_HEAD,
    tree: programme.PR380_TREE,
    base_ref: 'main',
    base_sha: programme.PR380_BASE_SHA,
    candidate: {
      repository,
      branch: programme.PR380_BRANCH,
      base_ref: 'main',
      base_sha: programme.PR380_BASE_SHA,
      head: programme.PR380_HEAD,
      tree: programme.PR380_TREE,
      version: '2.10.8',
    },
    accepted_evidence_ref: programme.FINAL_G4_EVIDENCE_REF,
  };
}
function finalisationCollector() {
  return {
    kind: 'WEB_AUTHENTICATED_GITHUB_COLLECTION',
    identity: 'github-web-readonly-adapter',
    version: 'v1',
    authenticated: true,
    provider_client_used: false,
  };
}
function finalisationSourceBinding(parent, child, pr379State, revisions) {
  const pr379FactsValue = finalisationPr379Facts(pr379State);
  const pr380FactsValue = finalisationPr380Facts();
  return programme.postMergeEpochFinalisation.buildSourceBinding({
    canonical_main: {
      ref: 'main',
      sha: programme.MERGE_COMMIT_SHA,
      tree: programme.MERGE_COMMIT_TREE,
      equals_merge_commit: true,
      complete: true,
    },
    child: {
      body_digest: child.body_digest,
      canonical_digest: child.canonical_digest,
      dependencies: clone(child.dependencies),
      issue: child.issue,
      labels: clone(child.labels),
      native_parent: child.native_parent,
      prefix_digest: child.prefix_digest,
      projection_digest: child.projection.projection_digest,
      relationships: clone(child.relationships),
      revision: revisions.child,
      sole_current: child.sole_current,
      suffix_digest: child.suffix_digest,
    },
    collector: finalisationCollector(),
    complete: true,
    parent: {
      body_digest: parent.body_digest,
      canonical_digest: parent.canonical_digest,
      issue: parent.issue,
      native_children: clone(parent.native_children),
      prefix_digest: parent.prefix_digest,
      relationships: clone(parent.relationships),
      revision: revisions.parent,
      suffix_digest: parent.suffix_digest,
    },
    pr_379: {
      body_digest: digestText('synthetic provider body #379'),
      facts: pr379FactsValue,
      facts_digest: programme.digestValue(pr379FactsValue),
      pr: 379,
      revision: revisions.pr379,
    },
    pr_380: {
      body_digest: digestText('synthetic provider body #380'),
      facts: pr380FactsValue,
      facts_digest: programme.digestValue(pr380FactsValue),
      pr: 380,
      revision: revisions.pr380,
    },
  });
}
function finalisationFixtureContext(revisionOverrides = {}) {
  const fixtures = finalisationStateFixtures();
  const revisions = finalisationRevisionSet(revisionOverrides);
  const bodyByCheckpoint = {
    BEFORE_STAGE_A: [fixtures.sourceParent, fixtures.sourceChild, fixtures.sourceState, 'OPEN'],
    CHILD_STAGE_A_OBSERVED: [fixtures.sourceParent, fixtures.stageAChild, fixtures.sourceState, 'OPEN'],
    PARENT_STAGE_A_OBSERVED: [fixtures.stageAParent, fixtures.stageAChild, fixtures.stageA, 'OPEN'],
    PR379_CLOSED_STAGE_A: [fixtures.stageAParent, fixtures.stageAChild, fixtures.stageA, 'CLOSED'],
    CHILD_STAGE_B_OBSERVED: [fixtures.stageAParent, fixtures.stageBChild, fixtures.stageA, 'CLOSED'],
    FINAL_TARGET_OBSERVED: [fixtures.stageBParent, fixtures.stageBChild, fixtures.stageB, 'CLOSED'],
  };
  const bindings = {};
  const evidenceParts = {};
  for (const checkpoint of Object.keys(bodyByCheckpoint)) {
    const [parentParsed, childParsed, parentState, pr379State] = bodyByCheckpoint[checkpoint];
    const childState = childParsed === fixtures.sourceChild ? fixtures.sourceState : childParsed === fixtures.stageAChild ? fixtures.stageA : fixtures.stageB;
    const parts = {
      parent: finalisationBodyEvidence(parentParsed, parentState, 'parent', revisions[checkpoint].parent),
      child: finalisationBodyEvidence(childParsed, childState, 'child', revisions[checkpoint].child),
      pr379State,
    };
    evidenceParts[checkpoint] = parts;
    bindings[checkpoint] = finalisationSourceBinding(parts.parent, parts.child, pr379State, revisions[checkpoint]);
    assert.ok(bindings[checkpoint], `${checkpoint}: source binding fixture`);
  }
  return { fixtures, revisions, bindings, evidenceParts };
}
function finalisationDecisionWithRevisions(revisionOverrides = {}) {
  const context = finalisationFixtureContext(revisionOverrides);
  const authority = finalisationAuthority();
  const value = programme.postMergeEpochFinalisation.createDecision({
    write_authority: authority,
    source_snapshot: context.bindings.BEFORE_STAGE_A,
  });
  return { context, decision: value };
}
function finalisationBodyEvidence(parsed, state, kind, revision) {
  const rawBody = parsed.prefix + parsed.managed + parsed.suffix;
  const value = kind === 'parent'
    ? {
      issue: 240,
      raw_body: rawBody,
      body_digest: parsed.body_digest,
      canonical_digest: programme.digestValue(state),
      revision,
      state: clone(state),
      native_children: [358, 359, 360, 361, 362, 363],
      relationships: { child_issue: 359, child_is_native_sub_issue: true, parent_issue: 240, sole_current: true },
      prefix_digest: parsed.prefix_digest,
      suffix_digest: parsed.suffix_digest,
      complete: true,
    }
    : {
      issue: 359,
      raw_body: rawBody,
      body_digest: parsed.body_digest,
      canonical_digest: programme.digestValue(state),
      revision,
      labels: ['current'],
      native_parent: 240,
      relationships: { child_issue: 359, child_is_native_sub_issue: true, parent_issue: 240, sole_current: true },
      sole_current: true,
      dependencies: [],
      state: snapshotState(state),
      projection: clone(parsed.envelope),
      prefix_digest: parsed.prefix_digest,
      suffix_digest: parsed.suffix_digest,
      complete: true,
    };
  return value;
}
function finalisationTransaction(checkpoint, decisionValue, context = finalisationFixtureContext()) {
  const ack = FINALISATION_ACK_SPECS[checkpoint] || null;
  const spec = ack || FINALISATION_NORMAL_SPECS[checkpoint];
  const previousCheckpoint = ack ? ack.pre : spec.previous;
  return {
    checkpoint,
    acknowledgement: ack ? 'LOST' : 'CONFIRMED',
    acknowledgement_loss_operation_order: ack ? ack.order : null,
    completed_operation_orders: ack ? ack.completed : spec.completed,
    continuation: previousCheckpoint === null ? null : {
      previous_checkpoint: previousCheckpoint,
      decision_digest: programme.digestValue(decisionValue),
      authority_body_digest: decisionValue.write_authority.body_digest,
      fresh_complete_rebind: true,
      exact_readback: true,
      complete: true,
    },
    readback: { complete: true, exact: true },
    duplicate_event_count: 1,
    previous_source_binding: previousCheckpoint === null ? null : clone(context.bindings[previousCheckpoint]),
    complete: true,
  };
}
function makeFinalisationEvidence(checkpoint = 'BEFORE_STAGE_A', options = {}) {
  const { context, decision: decisionValue } = finalisationDecisionWithRevisions(options.revision_overrides || {});
  assert.ok(decisionValue);
  const ack = FINALISATION_ACK_SPECS[checkpoint] || null;
  const observedCheckpoint = ack ? (options.ack_effect === false ? ack.pre : ack.base) : checkpoint;
  const parts = context.evidenceParts[observedCheckpoint];
  const revisions = context.revisions[observedCheckpoint];
  const fixtures = context.fixtures;
  const pr379State = parts.pr379State;
  assert.ok(parts.parent && parts.child && pr379State);
  const pr379FactsValue = finalisationPr379Facts(pr379State);
  const pr380FactsValue = finalisationPr380Facts();
  const pr379 = {
    ...pr379FactsValue,
    body_digest: digestText('synthetic provider body #379'),
    facts: pr379FactsValue,
    facts_digest: programme.digestValue(pr379FactsValue),
    revision: revisions.pr379,
    complete: true,
  };
  const pr380 = {
    ...pr380FactsValue,
    body_digest: digestText('synthetic provider body #380'),
    facts: pr380FactsValue,
    facts_digest: programme.digestValue(pr380FactsValue),
    revision: revisions.pr380,
    complete: true,
  };
  const canonicalMain = { ref: 'main', sha: programme.MERGE_COMMIT_SHA, tree: programme.MERGE_COMMIT_TREE, equals_merge_commit: true, complete: true };
  const authority = finalisationAuthority();
  const evidence = {
    schema: programme.FINALISATION_EVIDENCE_SCHEMA,
    root: programme.FINALISATION_ROOT,
    lock: programme.FINALISATION_LOCK,
    decision_digest: programme.digestValue(decisionValue),
    repository,
    parent_issue: 240,
    child_issue: 359,
    parent: clone(parts.parent),
    child: clone(parts.child),
    source_binding: clone(context.bindings[observedCheckpoint]),
    pr_379: pr379,
    pr_380: pr380,
    accepted_evidence: clone(decisionValue.accepted_evidence),
    accepted_epoch_event: {
      child_issue: 359,
      disposition: 'ACCEPTED',
      epoch_id: 'E3',
      evidence_ref: programme.FINAL_G4_EVIDENCE_REF,
      gate: 'G4',
      id: 'e3-post-merge-epoch-finalisation',
      occurrence: 1,
      complete: true,
    },
    merge_ancestry: {
      merged: true,
      method: 'MERGE_COMMIT',
      merge_commit: programme.MERGE_COMMIT_SHA,
      ordered_parents: [programme.MAIN_SHA, programme.PR380_HEAD],
      accepted_head: programme.PR380_HEAD,
      accepted_head_tree: programme.PR380_TREE,
      merge_tree: programme.MERGE_COMMIT_TREE,
    },
    canonical_main: canonicalMain,
    source_revisions: {
      canonical_digest: programme.FINALISATION_SOURCE_CANONICAL_DIGEST || programme.TARGET_CANONICAL_DIGEST,
      parent: { body_digest: fixtures.sourceParent.body_digest, prefix_digest: fixtures.sourceParent.prefix_digest, revision: context.revisions.BEFORE_STAGE_A.parent, suffix_digest: fixtures.sourceParent.suffix_digest },
      child: { body_digest: fixtures.sourceChild.body_digest, prefix_digest: fixtures.sourceChild.prefix_digest, revision: context.revisions.BEFORE_STAGE_A.child, suffix_digest: fixtures.sourceChild.suffix_digest },
      complete: true,
    },
    write_authority: authority,
    provider_facts: {
      inventory: 'EXACT_BOUNDED',
      provider_client_used: false,
      provider_cas_claim: false,
      facts_digest: context.bindings[observedCheckpoint].snapshot_digest,
      complete: true,
    },
    collector: {
      kind: 'WEB_AUTHENTICATED_GITHUB_COLLECTION',
      identity: 'github-web-readonly-adapter',
      version: 'v1',
      authenticated: true,
      provider_client_used: false,
    },
    transaction: finalisationTransaction(checkpoint, decisionValue, context),
    pagination: null,
    evidence_digest: null,
  };
  evidence.pagination = programme.postMergeEpochFinalisation.buildPaginationEvidence(evidence);
  return withEvidenceDigest(evidence);
}
function replaceFinalisationEvidence(value, pathParts, replacement) {
  const next = clone(value);
  let cursor = next;
  for (let index = 0; index < pathParts.length - 1; index += 1) cursor = cursor[pathParts[index]];
  cursor[pathParts[pathParts.length - 1]] = replacement;
  return withEvidenceDigest(next);
}
function gitChangedPaths(args) {
  try {
    return execFileSync('git', args, { encoding: 'utf8' })
      .split(/\r?\n/)
      .map((item) => item.trim())
      .filter(Boolean);
  } catch (_error) {
    return [];
  }
}
function sourceBootstrapRepinPending() {
  const sourcePaths = new Set([
    'repo/contracts/github-program-reconciler/programme-surface-contract-v5.json',
    'repo/contracts/github-program-reconciler/post-merge-epoch-finalisation-decision-v1.schema.json',
    'repo/contracts/github-program-reconciler/post-merge-epoch-finalisation-evidence-v1.schema.json',
    'repo/scripts/toolkit-github-program-state-v5.cjs',
    'repo/tests/toolkit-github-programme-projection-bootstrap-recovery-v5.test.cjs',
  ]);
  const workingTreePaths = [
    ...gitChangedPaths(['diff', '--name-only']),
    ...gitChangedPaths(['diff', '--cached', '--name-only']),
  ];
  const paths = workingTreePaths.length > 0 ? workingTreePaths : gitChangedPaths(['diff', '--name-only', 'HEAD^', 'HEAD']);
  return paths.length > 0
    && paths.every((item) => sourcePaths.has(item))
    && paths.includes('repo/contracts/github-program-reconciler/programme-surface-contract-v5.json');
}
function rebindFinalisationProviderRevision(value, resource, revision) {
  const next = clone(value);
  if (!['parent', 'child', 'pr_379', 'pr_380'].includes(resource)) throw new Error(`unknown finalisation resource: ${resource}`);
  next[resource].revision = revision;
  next.source_binding[resource].revision = revision;
  delete next.source_binding.snapshot_digest;
  next.source_binding = programme.postMergeEpochFinalisation.buildSourceBinding(next.source_binding);
  assert.ok(next.source_binding, `${resource}: rebuilt source binding`);
  next.provider_facts.facts_digest = next.source_binding.snapshot_digest;
  next.pagination = programme.postMergeEpochFinalisation.buildPaginationEvidence(next);
  return withEvidenceDigest(next);
}
function untrustedFinalisationSourceBinding(value) {
  const binding = {
    canonical_main: clone(value.canonical_main),
    child: {
      body_digest: value.child.body_digest,
      canonical_digest: value.child.canonical_digest,
      dependencies: clone(value.child.dependencies),
      issue: value.child.issue,
      labels: clone(value.child.labels),
      native_parent: value.child.native_parent,
      prefix_digest: value.child.prefix_digest,
      projection_digest: value.child.projection.projection_digest,
      relationships: clone(value.child.relationships),
      revision: value.child.revision,
      sole_current: value.child.sole_current,
      suffix_digest: value.child.suffix_digest,
    },
    collector: clone(value.collector),
    complete: true,
    parent: {
      body_digest: value.parent.body_digest,
      canonical_digest: value.parent.canonical_digest,
      issue: value.parent.issue,
      native_children: clone(value.parent.native_children),
      prefix_digest: value.parent.prefix_digest,
      relationships: clone(value.parent.relationships),
      revision: value.parent.revision,
      suffix_digest: value.parent.suffix_digest,
    },
    pr_379: {
      body_digest: value.pr_379.body_digest,
      facts: clone(value.pr_379.facts),
      facts_digest: value.pr_379.facts_digest,
      pr: value.pr_379.pr,
      revision: value.pr_379.revision,
    },
    pr_380: {
      body_digest: value.pr_380.body_digest,
      facts: clone(value.pr_380.facts),
      facts_digest: value.pr_380.facts_digest,
      pr: value.pr_380.pr,
      revision: value.pr_380.revision,
    },
  };
  return { ...binding, snapshot_digest: programme.digestValue(binding) };
}
function refreshFinalisationBodyEvidence(value, kind) {
  const parsed = kind === 'parent'
    ? programme.parseParentV5Body(value.parent.raw_body, { repository, parent_issue: 240 })
    : programme.parseChildV5Body(value.child.raw_body, { repository, parent_issue: 240 });
  assert.equal(parsed.ok, true, `${kind}: refreshed body parse`);
  const target = value[kind];
  target.body_digest = parsed.body_digest;
  target.prefix_digest = parsed.prefix_digest;
  target.suffix_digest = parsed.suffix_digest;
  target.canonical_digest = parsed.envelope.canonical_digest;
  if (kind === 'parent') target.state = clone(parsed.state);
  else target.projection = clone(parsed.envelope);
}
function recomputeFinalisationEvidence(value) {
  const next = clone(value);
  next.pr_379.facts_digest = programme.digestValue(next.pr_379.facts);
  next.pr_380.facts_digest = programme.digestValue(next.pr_380.facts);
  next.source_binding = untrustedFinalisationSourceBinding(next);
  next.provider_facts.facts_digest = next.source_binding.snapshot_digest;
  next.pagination = programme.postMergeEpochFinalisation.buildPaginationEvidence(next);
  return withEvidenceDigest(next);
}
function mutateFinalisationBody(value, kind, transform) {
  const next = clone(value);
  next[kind].raw_body = transform(next[kind].raw_body);
  refreshFinalisationBodyEvidence(next, kind);
  return recomputeFinalisationEvidence(next);
}
function rebaseFinalisationPr380(value) {
  const next = clone(value);
  const rebasedFacts = { ...next.pr_380.facts, merge_commit: 'f'.repeat(40) };
  next.pr_380.revision = 'synthetic:pr380:rebased';
  next.pr_380.body_digest = digestText('synthetic rebased provider body #380');
  next.pr_380.facts = rebasedFacts;
  next.pr_380.facts_digest = programme.digestValue(rebasedFacts);
  if (next.transaction.previous_source_binding) {
    const previous = clone(next.transaction.previous_source_binding);
    previous.pr_380.revision = next.pr_380.revision;
    previous.pr_380.body_digest = next.pr_380.body_digest;
    previous.pr_380.facts = clone(rebasedFacts);
    previous.pr_380.facts_digest = next.pr_380.facts_digest;
    delete previous.snapshot_digest;
    next.transaction.previous_source_binding = {
      ...previous,
      snapshot_digest: programme.digestValue(previous),
    };
  }
  return recomputeFinalisationEvidence(next);
}
function rebaseFinalisationPr380RevisionBody(value) {
  const next = clone(value);
  const revision = 'synthetic:pr380:rebased-shape';
  const bodyDigest = digestText('synthetic rebased provider body #380 without fact movement');
  next.pr_380.revision = revision;
  next.pr_380.body_digest = bodyDigest;
  if (next.transaction.previous_source_binding) {
    const previous = clone(next.transaction.previous_source_binding);
    previous.pr_380.revision = revision;
    previous.pr_380.body_digest = bodyDigest;
    delete previous.snapshot_digest;
    next.transaction.previous_source_binding = {
      ...previous,
      snapshot_digest: programme.digestValue(previous),
    };
  }
  return recomputeFinalisationEvidence(next);
}
function rebasePreviousBinding(value, resource, changes) {
  const next = clone(value);
  const previous = clone(next.transaction.previous_source_binding);
  assert.ok(previous, `${resource}: previous binding exists`);
  Object.assign(previous[resource], changes);
  if (resource === 'pr_379' || resource === 'pr_380') {
    previous[resource].facts_digest = programme.digestValue(previous[resource].facts);
  }
  delete previous.snapshot_digest;
  next.transaction.previous_source_binding = {
    ...previous,
    snapshot_digest: programme.digestValue(previous),
  };
  return withEvidenceDigest(next);
}
const DECISION_SCHEMA_TYPES = new Set(['null', 'boolean', 'object', 'array', 'number', 'integer', 'string']);
const SUPPORTED_DECISION_SCHEMA_KEYWORDS = new Set([
  '$schema', '$id', '$ref', 'title', 'type', 'const', 'enum', 'required', 'properties',
  'additionalProperties', 'items', 'minItems', 'maxItems', 'minimum', 'pattern', 'anyOf',
  'minLength', 'maxLength',
]);
function isSchemaRecord(value) { return value !== null && typeof value === 'object' && !Array.isArray(value); }
function deepEqual(left, right) {
  if (left === right) return true;
  if (typeof left !== typeof right || left === null || right === null) return false;
  if (Array.isArray(left) || Array.isArray(right)) {
    return Array.isArray(left) && Array.isArray(right)
      && left.length === right.length
      && left.every((item, index) => deepEqual(item, right[index]));
  }
  if (typeof left !== 'object') return false;
  const leftKeys = Object.keys(left).sort();
  const rightKeys = Object.keys(right).sort();
  return leftKeys.length === rightKeys.length
    && leftKeys.every((key, index) => key === rightKeys[index] && deepEqual(left[key], right[key]));
}
function resolvePublishedSchemaReference(reference, rootSchema, schemaPath) {
  if (typeof reference !== 'string' || reference.length === 0) {
    throw new Error(`Invalid schema $ref at ${schemaPath}`);
  }
  if (reference.startsWith('#')) {
    let resolved = rootSchema;
    const tokens = reference.slice(1).split('/').filter(Boolean);
    for (const token of tokens) {
      const key = token.replace(/~1/g, '/').replace(/~0/g, '~');
      if ((isSchemaRecord(resolved) || Array.isArray(resolved)) && Object.prototype.hasOwnProperty.call(resolved, key)) {
        resolved = resolved[key];
      } else {
        throw new Error(`Unresolved schema $ref ${reference} at ${schemaPath}`);
      }
    }
    return resolved;
  }
  if (reference === 'programme-state-v5.schema.json') return publishedProgrammeStateSchema;
  throw new Error(`Unsupported external schema $ref ${reference} at ${schemaPath}`);
}
function assertDecisionSchemaSupported(schema, schemaPath = '$', rootSchema = schema, refStack = new Set()) {
  if (!isSchemaRecord(schema)) throw new Error(`Unsupported decision schema at ${schemaPath}`);
  for (const keyword of Object.keys(schema)) {
    if (!SUPPORTED_DECISION_SCHEMA_KEYWORDS.has(keyword)) {
      throw new Error(`Unsupported decision schema keyword ${keyword} at ${schemaPath}`);
    }
  }
  if (Object.prototype.hasOwnProperty.call(schema, '$ref')) {
    if (Object.keys(schema).length !== 1) throw new Error(`Unsupported schema $ref sibling at ${schemaPath}`);
    const refKey = `${rootSchema.$id || 'root'}:${schema.$ref}`;
    if (refStack.has(refKey)) throw new Error(`Cyclic schema $ref at ${schemaPath}`);
    const target = resolvePublishedSchemaReference(schema.$ref, rootSchema, schemaPath);
    assertDecisionSchemaSupported(target, `${schemaPath}.$ref(${schema.$ref})`, rootSchema, new Set([...refStack, refKey]));
    return;
  }
  if (Object.prototype.hasOwnProperty.call(schema, 'type')) {
    const types = Array.isArray(schema.type) ? schema.type : [schema.type];
    if (types.length === 0 || types.some((type) => typeof type !== 'string' || !DECISION_SCHEMA_TYPES.has(type))) {
      throw new Error(`Invalid decision schema type at ${schemaPath}`);
    }
  }
  if (Object.prototype.hasOwnProperty.call(schema, 'enum') && !Array.isArray(schema.enum)) {
    throw new Error(`Invalid decision schema enum at ${schemaPath}`);
  }
  if (Object.prototype.hasOwnProperty.call(schema, 'required')
    && (!Array.isArray(schema.required) || schema.required.some((key) => typeof key !== 'string'))) {
    throw new Error(`Invalid decision schema required at ${schemaPath}`);
  }
  if (Object.prototype.hasOwnProperty.call(schema, 'properties') && !isSchemaRecord(schema.properties)) {
    throw new Error(`Invalid decision schema properties at ${schemaPath}`);
  }
  if (Object.prototype.hasOwnProperty.call(schema, 'additionalProperties') && schema.additionalProperties !== false) {
    throw new Error(`Unsupported decision schema additionalProperties at ${schemaPath}`);
  }
  for (const keyword of ['minItems', 'maxItems']) {
    if (Object.prototype.hasOwnProperty.call(schema, keyword)
      && (!Number.isSafeInteger(schema[keyword]) || schema[keyword] < 0)) {
      throw new Error(`Invalid decision schema ${keyword} at ${schemaPath}`);
    }
  }
  if (Object.prototype.hasOwnProperty.call(schema, 'minimum')
    && (typeof schema.minimum !== 'number' || !Number.isFinite(schema.minimum))) {
    throw new Error(`Invalid decision schema minimum at ${schemaPath}`);
  }
  for (const keyword of ['minLength', 'maxLength']) {
    if (Object.prototype.hasOwnProperty.call(schema, keyword)
      && (!Number.isSafeInteger(schema[keyword]) || schema[keyword] < 0)) {
      throw new Error(`Invalid decision schema ${keyword} at ${schemaPath}`);
    }
  }
  if (Object.prototype.hasOwnProperty.call(schema, 'pattern')) {
    if (typeof schema.pattern !== 'string') throw new Error(`Invalid decision schema pattern at ${schemaPath}`);
    try { new RegExp(schema.pattern); } catch (_error) { throw new Error(`Invalid decision schema pattern at ${schemaPath}`); }
  }
  if (schema.properties) {
    for (const [property, propertySchema] of Object.entries(schema.properties)) {
      assertDecisionSchemaSupported(propertySchema, `${schemaPath}.properties.${property}`, rootSchema, refStack);
    }
  }
  if (Object.prototype.hasOwnProperty.call(schema, 'anyOf')) {
    if (!Array.isArray(schema.anyOf) || schema.anyOf.length === 0) throw new Error(`Invalid decision schema anyOf at ${schemaPath}`);
    schema.anyOf.forEach((branch, index) => assertDecisionSchemaSupported(branch, `${schemaPath}.anyOf[${index}]`, rootSchema, refStack));
  }
  if (Object.prototype.hasOwnProperty.call(schema, 'items')) {
    assertDecisionSchemaSupported(schema.items, `${schemaPath}.items`, rootSchema, refStack);
  }
}
function matchesDecisionSchemaType(value, type) {
  if (type === 'null') return value === null;
  if (type === 'boolean') return typeof value === 'boolean';
  if (type === 'object') return isSchemaRecord(value);
  if (type === 'array') return Array.isArray(value);
  if (type === 'number') return typeof value === 'number' && Number.isFinite(value);
  if (type === 'integer') return Number.isSafeInteger(value);
  if (type === 'string') return typeof value === 'string';
  return false;
}
function validateDecisionSchemaNode(value, schema, rootSchema = schema, schemaPath = '$', refStack = new Set()) {
  if (schema.$ref !== undefined) {
    if (Object.keys(schema).length !== 1) return false;
    const refKey = `${rootSchema.$id || 'root'}:${schema.$ref}`;
    if (refStack.has(refKey)) return false;
    let target;
    try {
      target = resolvePublishedSchemaReference(schema.$ref, rootSchema, schemaPath);
    } catch (_error) {
      return false;
    }
    return validateDecisionSchemaNode(value, target, rootSchema, `${schemaPath}.$ref(${schema.$ref})`, new Set([...refStack, refKey]));
  }
  if (schema.anyOf !== undefined && !schema.anyOf.some((branch) => validateDecisionSchemaNode(value, branch, rootSchema, `${schemaPath}.anyOf`, refStack))) return false;
  if (schema.type !== undefined) {
    const types = Array.isArray(schema.type) ? schema.type : [schema.type];
    if (!types.some((type) => matchesDecisionSchemaType(value, type))) return false;
  }
  if (Object.prototype.hasOwnProperty.call(schema, 'const') && !deepEqual(value, schema.const)) return false;
  if (schema.enum !== undefined && !schema.enum.some((allowed) => deepEqual(value, allowed))) return false;
  if (isSchemaRecord(value)) {
    if (schema.required !== undefined && schema.required.some((key) => !Object.prototype.hasOwnProperty.call(value, key))) return false;
    if (schema.additionalProperties === false
      && Object.keys(value).some((key) => !schema.properties || !Object.prototype.hasOwnProperty.call(schema.properties, key))) return false;
    if (schema.properties !== undefined) {
      for (const [property, propertySchema] of Object.entries(schema.properties)) {
        if (Object.prototype.hasOwnProperty.call(value, property)
          && !validateDecisionSchemaNode(value[property], propertySchema, rootSchema, `${schemaPath}.properties.${property}`, refStack)) return false;
      }
    }
  }
  if (Array.isArray(value)) {
    if (schema.minItems !== undefined && value.length < schema.minItems) return false;
    if (schema.maxItems !== undefined && value.length > schema.maxItems) return false;
    if (schema.items !== undefined
      && value.some((item) => !validateDecisionSchemaNode(item, schema.items, rootSchema, `${schemaPath}.items`, refStack))) return false;
  }
  if (typeof value === 'number' && schema.minimum !== undefined && value < schema.minimum) return false;
  if (typeof value === 'string') {
    if (schema.minLength !== undefined && value.length < schema.minLength) return false;
    if (schema.maxLength !== undefined && value.length > schema.maxLength) return false;
    if (schema.pattern !== undefined && !new RegExp(schema.pattern).test(value)) return false;
  }
  return true;
}
function validateDecisionAgainstPublishedSchema(value, schema) {
  assertDecisionSchemaSupported(schema);
  return validateDecisionSchemaNode(value, schema);
}
function parsedSources() {
  assert.equal(typeof PARENT_BODY, 'string');
  assert.equal(typeof CHILD_BODY, 'string');
  const parent = programme.parseParentV5Body(PARENT_BODY, { repository, parent_issue: 240 });
  const child = programme.parseChildV5Body(CHILD_BODY, { repository, parent_issue: 240 });
  assert.equal(parent.ok, true, parent.code);
  assert.equal(child.ok, true, child.code);
  return { parent, child };
}
function pagination(evidence) {
  return Object.fromEntries(programme.PAGINATION_KEYS.map((key) => [
    key,
    programme.buildPaginationEvidence(key, evidence),
  ]));
}
function webAuthority() {
  const records = [...decision.web_authority.controlling, ...decision.web_authority.predecessor];
  return records.map((entry) => ({
    issue: entry.issue,
    comment_id: entry.comment_id,
    body: entry.issue === 379
      ? PR379_COMMENT_BODIES[String(entry.comment_id)]
      : entry.comment_id === 5580972753 || entry.comment_id === 5580530088
        ? PR379_COMMENT_BODIES['issue-359-' + String(entry.comment_id)]
        : PR379_COMMENT_BODIES['issue-240-' + String(entry.comment_id)],
    body_digest: entry.body_digest,
  }));
}
function pr379Facts() {
  return {
    repository,
    pr: 379,
    state: 'OPEN',
    draft: true,
    merged: false,
    merged_at: null,
    head: programme.FROZEN_HEAD,
    tree: programme.FROZEN_TREE,
    branch: programme.FROZEN_BRANCH,
    base_ref: 'main',
    base_sha: programme.MAIN_SHA,
    changed_files: 48,
    candidate: clone(decision.pr_379.candidate),
    reviews: [{
      ...clone(programme.PR379_REVIEW_FACTS[0]),
      body: PR379_REVIEW_BODY,
    }],
    threads: [],
    comments: programme.PR379_COMMENT_FACTS.map((entry) => ({
      ...clone(entry),
      body: PR379_COMMENT_BODIES[String(entry.id)],
    })),
    checks: clone(programme.PR379_CHECK_FACTS),
    facts_digest: decision.pr_379.facts_digest,
    complete: true,
  };
}
function pr366Facts() {
  return {
    pr: 366,
    status: 'RETIRED',
    github_state: 'CLOSED',
    draft: true,
    merged: false,
    merged_at: null,
    merge_commit: null,
    role: 'INTERMEDIATE',
    completes_child: false,
    candidate: null,
    head: programme.PR366_HEAD,
    tree: programme.PR366_TREE,
    base_ref: 'main',
    base_sha: programme.PR366_BASE_SHA,
    complete: true,
  };
}
function snapshotState(state) {
  const child = state.children.find((item) => item.issue === 359);
  const lane = state.active_lanes.find((item) => item.child_issue === 359);
  return {
    lifecycle: child.lifecycle,
    finality: child.finality.state,
    gate_state: state.recovery ? 'HELD' : lane ? lane.gate_state : 'NONE',
    normal_active_lanes: state.active_lanes.length,
    active_blocking_recovery_hold: Boolean(state.recovery),
  };
}
function makeEvidence(snapshot = 'BEFORE_CHILD', continuation = null) {
  const source = parsedSources();
  const targetState = programme.buildRecoveryTargetState(source.parent.state);
  assert.ok(targetState);
  const targetRendered = programme.renderProgrammeV5(targetState);
  assert.equal(targetRendered.ok, true, targetRendered.code);
  const targetParent = programme.parseParentV5Body(targetRendered.parent, { repository, parent_issue: 240 });
  const targetChild = programme.parseChildV5Body(targetRendered.child, { repository, parent_issue: 240 });
  assert.equal(targetParent.ok, true, targetParent.code);
  assert.equal(targetChild.ok, true, targetChild.code);
  const useTargetParent = snapshot === 'PARENT_AND_CHILD_TARGET_OBSERVED';
  const useTargetChild = snapshot !== 'BEFORE_CHILD';
  const parent = useTargetParent ? targetParent : source.parent;
  const child = useTargetChild ? targetChild : source.child;
  const parentState = useTargetParent ? targetState : source.parent.state;
  const childState = useTargetParent || useTargetChild ? snapshotState(targetState) : snapshotState(source.parent.state);
  const evidence = {
    schema: programme.EVIDENCE_SCHEMA,
    recovery_root: programme.RECOVERY_ROOT,
    lock: programme.LOCK,
    decision_digest: programme.digestValue(decision),
    snapshot,
    repository,
    parent_issue: 240,
    child_issue: 359,
    parent: {
      issue: 240,
      raw_body: useTargetParent ? targetRendered.parent : PARENT_BODY,
      body_digest: parent.body_digest,
      canonical_digest: parent.envelope.canonical_digest,
      revision: programme.SOURCE_PARENT_REVISION,
      state: clone(parentState),
      native_children: [358, 359, 360, 361, 362, 363],
      relationships: { child_issue: 359, child_is_native_sub_issue: true, parent_issue: 240, sole_current: true },
      prefix_digest: parent.prefix_digest,
      suffix_digest: parent.suffix_digest,
      complete: true,
    },
    child: {
      issue: 359,
      raw_body: useTargetChild ? targetRendered.child : CHILD_BODY,
      body_digest: child.body_digest,
      canonical_digest: child.envelope.canonical_digest,
      revision: programme.SOURCE_CHILD_REVISION,
      labels: ['current'],
      native_parent: 240,
      relationships: { child_issue: 359, child_is_native_sub_issue: true, parent_issue: 240, sole_current: true },
      sole_current: true,
      dependencies: [],
      state: childState,
      projection: clone(child.envelope),
      prefix_digest: child.prefix_digest,
      suffix_digest: child.suffix_digest,
      complete: true,
    },
    pr_366: pr366Facts(),
    pr_379: pr379Facts(),
    web_authority: webAuthority(),
    pagination: null,
    provider_evidence: {
      check_runs: {
        endpoint_or_query_identity: programme.PAGINATION_COLLECTIONS.checks.endpoint,
        field: programme.CHECK_RUNS_TOTAL_FIELD,
        value: 6,
      },
    },
    collector: {
      kind: 'WEB_AUTHENTICATED_GITHUB_COLLECTION',
      identity: 'github-web-readonly-adapter',
      version: 'v1',
      authenticated: true,
      provider_client_used: false,
    },
    authority_digest: decision.web_authority.digest,
    continuation,
    evidence_digest: null,
  };
  evidence.pagination = pagination(evidence);
  return withEvidenceDigest(evidence);
}
function continuationFromPreview(preview) {
  const operation = preview.operations[0];
  return {
    authority_digest: preview.authority_digest,
    child_operation_digest: operation.receipt_logical_operation_digest,
    child_operation_id: operation.operation_id,
    decision_digest: preview.decision_digest,
    preview_id: preview.preview_id,
    receipt_operation_digest: operation.receipt_logical_operation_digest,
    receipt_operation_id: 'operation-recovery-test',
    safety_mode: programme.WRITE_SAFETY_MODE,
  };
}
function replaceEvidence(value, pathParts, replacement) {
  const next = clone(value);
  let cursor = next;
  for (let index = 0; index < pathParts.length - 1; index += 1) cursor = cursor[pathParts[index]];
  cursor[pathParts[pathParts.length - 1]] = replacement;
  return withEvidenceDigest(next);
}

test('locked source identity, exact stale digests, and parser coverage', () => {
  const source = parsedSources();
  assert.equal(source.parent.body_digest, programme.SOURCE_PARENT_BODY_DIGEST);
  assert.equal(source.child.body_digest, programme.SOURCE_CHILD_BODY_DIGEST);
  assert.equal(source.parent.envelope.canonical_digest, programme.SOURCE_CANONICAL_DIGEST);
  assert.equal(source.parent.envelope.canonical_digest, source.child.envelope.canonical_digest);
  assert.equal(source.parent.state.design_lock, 'DL-S2-GITHUB-PROGRAM-SURFACE-RECOVERY-003');
});

test('held CURRENT with zero normal lanes requires the eligible blocking hold', () => {
  const source = parsedSources();
  const target = programme.buildRecoveryTargetState(source.parent.state);
  assert.equal(programme.validateCanonicalStateV5(target).ok, true);
  const withoutHold = clone(target);
  withoutHold.children.find((item) => item.issue === 359).holds = [];
  assert.equal(programme.validateCanonicalStateV5(withoutHold).code, 'V5_CURRENT_ZERO_LANE_HOLD_REQUIRED');
  const withoutRecovery = clone(target);
  delete withoutRecovery.recovery;
  assert.equal(programme.validateCanonicalStateV5(withoutRecovery).code, 'V5_CURRENT_ZERO_LANE_HOLD_REQUIRED');
  const hold = target.children.find((item) => item.issue === 359).holds[0];
  assert.deepEqual(hold, {
    id: programme.RECOVERY_ROOT,
    root: programme.RECOVERY_ROOT,
    lock: programme.LOCK,
    kind: 'BLOCKING',
    scope: 'PROGRAMME_PROJECTION_RECOVERY',
    active: true,
    blocks_normal_lanes: true,
    evidence_ref: programme.HOLD_EVIDENCE_REF,
    summary: 'Managed v5 parent and child projections are stale and remain held pending separately authorised recovery.',
  });
  assert.equal(target.evidence_refs.some((item) => item.id === programme.HOLD_EVIDENCE_REF
    && item.kind === 'WEB' && item.reference === 'github:issue-comment:359:5580530088'), true);
  for (const field of ['root', 'lock', 'kind', 'scope', 'evidence_ref']) {
    const malformed = clone(target);
    delete malformed.children.find((item) => item.issue === 359).holds[0][field];
    assert.equal(programme.validateCanonicalStateV5(malformed).ok, false, field);
  }
  for (const [field, value] of [
    ['root', 'wrong-root'],
    ['lock', 'wrong-lock'],
    ['kind', 'ADVISORY'],
    ['scope', 'OTHER_SCOPE'],
    ['active', false],
    ['blocks_normal_lanes', false],
    ['evidence_ref', 'dangling-evidence'],
  ]) {
    const malformed = clone(target);
    malformed.children.find((item) => item.issue === 359).holds[0][field] = value;
    assert.equal(programme.validateCanonicalStateV5(malformed).ok, false, field);
  }
  const nonWebEvidence = clone(target);
  nonWebEvidence.evidence_refs.find((item) => item.id === programme.HOLD_EVIDENCE_REF).kind = 'COMMIT';
  assert.equal(programme.validateCanonicalStateV5(nonWebEvidence).ok, false);
  const conflictingHolds = clone(target);
  conflictingHolds.children.find((item) => item.issue === 359).holds.push(clone(hold));
  assert.equal(programme.validateCanonicalStateV5(conflictingHolds).ok, false);
});

test('fixed retained and historical PR semantics are exact', () => {
  const source = parsedSources();
  const target = programme.buildRecoveryTargetState(source.parent.state);
  const child = target.children.find((item) => item.issue === 359);
  assert.deepEqual(child.pr_registry[0], {
    accepted_evidence_ref: null,
    candidate: null,
    completes_child: false,
    draft: true,
    epoch_id: 'E3',
    github_state: 'CLOSED',
    merged: false,
    pr: 366,
    retention_evidence_ref: null,
    retirement_evidence_ref: programme.RECOVERY_EVIDENCE_REF,
    role: 'INTERMEDIATE',
    status: 'RETIRED',
  });
  assert.deepEqual(child.pr_registry[1], {
    accepted_evidence_ref: null,
    candidate: decision.pr_379.candidate,
    completes_child: false,
    draft: true,
    epoch_id: 'E3',
    github_state: 'OPEN',
    merged: false,
    pr: 379,
    retention_evidence_ref: programme.RETENTION_EVIDENCE_REF,
    retirement_evidence_ref: null,
    role: 'INTERMEDIATE',
    status: 'RETAINED',
  });
  const malformed = clone(target);
  malformed.children.find((item) => item.issue === 359).pr_registry[1].status = 'ACTIVE';
  assert.equal(programme.validateCanonicalStateV5(malformed).ok, false);
  const malformedCandidate = clone(target);
  delete malformedCandidate.children.find((item) => item.issue === 359).pr_registry[1].candidate.tree;
  assert.equal(programme.validateCanonicalStateV5(malformedCandidate).ok, false);
  for (const replacement of [null, 'dangling-retention-evidence', 'non-web-retention']) {
    const malformedRetention = clone(target);
    malformedRetention.children.find((item) => item.issue === 359).pr_registry[1].retention_evidence_ref = replacement;
    if (replacement === 'non-web-retention') {
      malformedRetention.evidence_refs.find((item) => item.id === programme.RETENTION_EVIDENCE_REF).kind = 'COMMIT';
    }
    assert.equal(programme.validateCanonicalStateV5(malformedRetention).ok, false, replacement);
  }
  const malformed366 = clone(target);
  malformed366.children.find((item) => item.issue === 359).pr_registry[0].retention_evidence_ref = programme.RETENTION_EVIDENCE_REF;
  assert.equal(programme.validateCanonicalStateV5(malformed366).ok, false);
  assert.equal(target.evidence_refs.some((item) => item.id === programme.RETENTION_EVIDENCE_REF
    && item.kind === 'WEB' && item.reference === 'github:issue-comment:379:5580538176'), true);
});

test('old root is typed terminal non-convergence and parked root is not launched', () => {
  const source = parsedSources();
  const target = programme.buildRecoveryTargetState(source.parent.state);
  assert.deepEqual(target.recovery.old_root, {
    root: programme.OLD_ROOT,
    disposition: 'NON_CONVERGENT',
    terminal: true,
    repair_budget: { used: 2, limit: 2, further_repair_authorised: false },
  });
  assert.deepEqual(target.recovery.parked_root, {
    root: programme.PARKED_ROOT,
    status: 'NOT_LAUNCHED',
  });
});

test('fixed semantic delta removes the normal lane and keeps unrelated queued children', () => {
  const source = parsedSources();
  const target = programme.buildRecoveryTargetState(source.parent.state);
  assert.deepEqual(target.active_lanes, []);
  assert.equal(target.children.find((item) => item.issue === 359).lifecycle, 'CURRENT');
  assert.deepEqual(target.children.filter((item) => item.issue >= 360).map((item) => item.lifecycle), ['QUEUED', 'QUEUED', 'QUEUED', 'QUEUED']);
  assert.deepEqual(target.children.find((item) => item.issue === 360).dependencies, source.parent.state.children.find((item) => item.issue === 360).dependencies);
  assert.equal(target.children.find((item) => item.issue === 359).finality.state, 'HELD');
});

test('decision is closed and rejects caller-controlled state shapes', () => {
  assert.equal(programme.validateDecision(decision).ok, true);
  const extra = { ...clone(decision), desired: {} };
  assert.equal(programme.validateDecision(extra).ok, false);
  const changed = clone(decision);
  changed.write_safety.provider_cas_claim = true;
  assert.equal(programme.validateDecision(changed).ok, false);
  assert.equal(Object.prototype.hasOwnProperty.call(decision, 'desired'), false);
});

test('runtime recovery decision conforms to the published schema and rejects fixed-field drift', () => {
  assert.equal(validateDecisionAgainstPublishedSchema(decision, publishedDecisionSchema), true);
  const rejectDrift = (label, mutate) => {
    const candidate = clone(decision);
    mutate(candidate);
    assert.equal(validateDecisionAgainstPublishedSchema(candidate, publishedDecisionSchema), false, label);
  };
  rejectDrift('recovery_root', (candidate) => { candidate.recovery_root = 'DRIFTED_RECOVERY_ROOT'; });
  rejectDrift('lock', (candidate) => { candidate.lock = 'DRIFTED_LOCK'; });
  rejectDrift('self_retirement_fence.target_canonical_digest', (candidate) => {
    candidate.self_retirement_fence.target_canonical_digest = 'f'.repeat(64);
  });
  rejectDrift('write_safety.mode', (candidate) => { candidate.write_safety.mode = 'DRIFTED_MODE'; });
  rejectDrift('write_safety.provider_cas_available', (candidate) => {
    candidate.write_safety.provider_cas_available = true;
  });
  rejectDrift('write_safety.provider_cas_claim', (candidate) => {
    candidate.write_safety.provider_cas_claim = true;
  });
  for (const index of decision.allowed_body_targets.map((_target, index) => index)) {
    rejectDrift(`allowed_body_targets[${index}].operation_kind`, (candidate) => {
      candidate.allowed_body_targets[index].operation_kind = 'DRIFTED_OPERATION_KIND';
    });
  }
  rejectDrift('prohibitions.provider_cas', (candidate) => { candidate.prohibitions.provider_cas = true; });
  rejectDrift('pr_379.status', (candidate) => { candidate.pr_379.status = 'DRIFTED_STATUS'; });
  const unsupportedSchema = clone(publishedDecisionSchema);
  unsupportedSchema.properties.pr_379.properties.status.future_validation_keyword = true;
  assert.throws(
    () => validateDecisionAgainstPublishedSchema(decision, unsupportedSchema),
    /Unsupported decision schema keyword future_validation_keyword/,
  );
});

test('successful stale preview has exactly two ordered body operations', () => {
  const evidence = makeEvidence();
  const preview = programme.projectionBootstrapRecovery.preview({ decision, evidence });
  assert.equal(preview.ok, true, preview.code);
  assert.equal(preview.operation_count, 2);
  assert.deepEqual(preview.operation_order, [359, 240]);
  assert.deepEqual(preview.operations.map((operation) => operation.issue), [359, 240]);
  assert.equal(preview.operations[0].order, 1);
  assert.equal(preview.operations[1].order, 2);
  assert.equal(preview.write_safety.mode, programme.WRITE_SAFETY_MODE);
  assert.equal(preview.write_safety.provider_cas_claim, false);
  assert.equal(preview.receipt.operation_kind, 'IDEMPOTENT_SET');
  assert.equal(preview.receipt.safety_class, 'IDEMPOTENT');
  assert.equal(preview.receipt.provider_cas_claim, false);
  assert.equal(preview.receipt.source_changed, false);
  assert.equal(preview.self_retirement_fence.exact_target_only, true);
});

test('standalone child sole_current must agree with nested relationship evidence', () => {
  const validEvidence = makeEvidence();
  const validValidation = programme.validateEvidence(validEvidence, decision);
  assert.equal(validValidation.ok, true, validValidation.code);
  const validPreview = programme.projectionBootstrapRecovery.preview({ decision, evidence: validEvidence });
  assert.equal(validPreview.ok, true, validPreview.code);
  assert.equal(validPreview.operation_count, 2);
  assert.deepEqual(validPreview.operation_order, [359, 240]);

  const contradictory = clone(validEvidence);
  contradictory.child.sole_current = false;
  contradictory.pagination.child = programme.buildPaginationEvidence('child', contradictory);
  const contradictoryEvidence = withEvidenceDigest(contradictory);
  assert.equal(contradictoryEvidence.child.relationships.sole_current, true);
  assert.equal(contradictoryEvidence.child.sole_current, false);
  assert.notEqual(contradictoryEvidence.evidence_digest, validEvidence.evidence_digest);
  for (const field of Object.keys(validEvidence.child)) {
    if (field !== 'sole_current') assert.deepEqual(contradictoryEvidence.child[field], validEvidence.child[field], field);
  }

  const validation = programme.validateEvidence(contradictoryEvidence, decision);
  assert.equal(validation.ok, false);
  assert.equal(validation.code, 'RECOVERY_CHILD_EVIDENCE_INVALID');
  const preview = programme.projectionBootstrapRecovery.preview({ decision, evidence: contradictoryEvidence });
  assert.equal(preview.ok, false);
  assert.equal(preview.code, 'RECOVERY_CHILD_EVIDENCE_INVALID');
  assert.equal(preview.operations, undefined);
});

test('source revision movement fails closed symmetrically while locked child evidence remains valid', () => {
  const locked = makeEvidence();
  const lockedValidation = programme.validateEvidence(locked, decision);
  assert.equal(lockedValidation.ok, true, lockedValidation.code);
  assert.equal(locked.child.revision, '2026-09-08T07:21:42Z');
  const lockedPreview = programme.projectionBootstrapRecovery.preview({ decision, evidence: locked });
  assert.equal(lockedPreview.ok, true, lockedPreview.code);
  assert.equal(lockedPreview.operation_count, 2);
  assert.deepEqual(lockedPreview.operation_order, [359, 240]);

  const parentMoved = clone(locked);
  parentMoved.parent.revision = '2026-09-08T07:22:00Z';
  parentMoved.pagination.parent = programme.buildPaginationEvidence('parent', parentMoved);
  const parentMovedEvidence = withEvidenceDigest(parentMoved);
  const parentMovedValidation = programme.validateEvidence(parentMovedEvidence, decision);
  assert.equal(parentMovedValidation.ok, false);
  assert.equal(parentMovedValidation.code, 'RECOVERY_PARENT_SOURCE_STALE');

  const childMoved = clone(locked);
  childMoved.child.revision = '2026-09-08T07:22:00Z';
  childMoved.pagination.child = programme.buildPaginationEvidence('child', childMoved);
  const childMovedEvidence = withEvidenceDigest(childMoved);
  for (const field of [
    'issue', 'raw_body', 'body_digest', 'canonical_digest', 'labels', 'native_parent',
    'relationships', 'sole_current', 'dependencies', 'state', 'projection',
    'prefix_digest', 'suffix_digest', 'complete',
  ]) assert.deepEqual(childMovedEvidence.child[field], locked.child[field], field);
  assert.equal(childMovedEvidence.child.revision, '2026-09-08T07:22:00Z');
  assert.notDeepEqual(childMovedEvidence.pagination.child, locked.pagination.child);
  const childMovedValidation = programme.validateEvidence(childMovedEvidence, decision);
  assert.equal(childMovedValidation.ok, false);
  assert.equal(childMovedValidation.code, 'RECOVERY_CHILD_SOURCE_STALE');
  const movedPreview = programme.projectionBootstrapRecovery.preview({ decision, evidence: childMovedEvidence });
  assert.equal(movedPreview.ok, false);
  assert.equal(movedPreview.code, 'RECOVERY_CHILD_SOURCE_STALE');
  assert.equal(movedPreview.operations, undefined);
});

test('identical inputs render and preview deterministically', () => {
  const source = parsedSources();
  const target = programme.buildRecoveryTargetState(source.parent.state);
  assert.deepEqual(programme.renderProgrammeV5(target), programme.renderProgrammeV5(target));
  const evidence = makeEvidence();
  assert.deepEqual(
    programme.projectionBootstrapRecovery.preview({ decision, evidence }),
    programme.projectionBootstrapRecovery.preview({ decision: clone(decision), evidence: clone(evidence) }),
  );
});

test('unrelated bytes outside managed blocks are preserved', () => {
  const source = parsedSources();
  const target = programme.buildRecoveryTargetState(source.parent.state);
  const rendered = programme.renderProgrammeV5(target);
  const parentWithOutside = 'owner-prefix\n' + rendered.parent + '\nowner-suffix';
  const childWithOutside = 'owner-prefix\n' + rendered.child + '\nowner-suffix';
  const parentParsed = programme.parseParentV5Body(parentWithOutside, { repository, parent_issue: 240 });
  const childParsed = programme.parseChildV5Body(childWithOutside, { repository, parent_issue: 240 });
  assert.equal(parentParsed.prefix, 'owner-prefix\n');
  assert.equal(parentParsed.suffix, '\nowner-suffix');
  assert.equal(childParsed.prefix, 'owner-prefix\n');
  assert.equal(childParsed.suffix, '\nowner-suffix');
  assert.equal(parentParsed.managed, rendered.parent);
  assert.equal(childParsed.managed, rendered.child);
});

test('incomplete, stale, contradictory, and unpaginated evidence fail closed', () => {
  const evidence = makeEvidence();
  assert.equal(programme.validateEvidence(evidence, decision).ok, true);
  assert.equal(programme.validateEvidence(replaceEvidence(evidence, ['pagination', 'comments', 'complete'], false), decision).ok, false);
  assert.equal(programme.validateEvidence(replaceEvidence(evidence, ['parent', 'complete'], false), decision).ok, false);
  assert.equal(programme.validateEvidence(replaceEvidence(evidence, ['parent', 'canonical_digest'], 'f'.repeat(64)), decision).ok, false);
  assert.equal(programme.validateEvidence(replaceEvidence(evidence, ['authority_digest'], 'f'.repeat(64)), decision).ok, false);
  assert.equal(programme.validateEvidence(replaceEvidence(evidence, ['collector', 'authenticated'], false), decision).ok, false);
});

test('pagination evidence proves identity, counts, ordered pages, progression, totals, and inventory', () => {
  const evidence = makeEvidence();
  const mutate = (pathParts, replacement) => programme.validateEvidence(replaceEvidence(evidence, pathParts, replacement), decision).ok;
  assert.equal(mutate(['pagination', 'parent', 'endpoint_or_query_identity'], 'github:issues/other'), false);
  assert.equal(mutate(['pagination', 'parent', 'page_size'], 50), false);
  assert.equal(mutate(['pagination', 'parent', 'page_count'], 2), false);
  assert.equal(mutate(['pagination', 'parent', 'retrieved_count'], 0), false);
  assert.equal(mutate(['pagination', 'parent', 'ordered_page_digests', 0, 'digest'], 'f'.repeat(64)), false);
  assert.equal(mutate(['pagination', 'parent', 'inventory_digest'], 'f'.repeat(64)), false);
  assert.equal(mutate(['pagination', 'native_children', 'progression', 'pages', 0, 'page'], 2), false);
  assert.equal(mutate(['pagination', 'parent', 'transport_mode'], 'LINK'), false);
  assert.equal(mutate(['pagination', 'parent', 'progression'], { style: 'LINK', pages: [{ page: 1, next_url: null }] }), false);
  assert.equal(mutate(['pagination', 'comments', 'terminal_state', 'has_next_page'], true), false);
  assert.equal(mutate(['pagination', 'comments', 'terminal_state', 'next_url'], 'https://api.github.com/issues?page=2'), false);
  assert.equal(mutate(['pagination', 'parent', 'server_total', 'value'], 999), false);
  assert.equal(mutate(['pagination', 'threads', 'server_total'], { status: 'AVAILABLE', value: 0 }), false);
  assert.equal(mutate(['pagination', 'threads', 'server_total'], { status: 'UNAVAILABLE' }), false);
  assert.equal(mutate(['pagination', 'review_thread_comments', 'complete'], false), false);
  const reordered = clone(evidence);
  reordered.pagination.parent.page_count = 2;
  reordered.pagination.parent.ordered_page_digests.push({ page: 2, digest: 'f'.repeat(64) });
  reordered.pagination.parent.progression = {
    style: 'LINK',
    pages: [
      { page: 1, next_url: 'https://api.github.com/issues?page=2' },
      { page: 2, next_url: null },
    ],
  };
  assert.equal(programme.validateEvidence(withEvidenceDigest(reordered), decision).ok, false);
  const completeOnly = clone(evidence);
  completeOnly.pagination.parent = { complete: true };
  assert.equal(programme.validateEvidence(withEvidenceDigest(completeOnly), decision).ok, false);
});

test('pagination transport and provider totals remain truthful', () => {
  const evidence = makeEvidence();
  const mutate = (pathParts, replacement) => programme.validateEvidence(replaceEvidence(evidence, pathParts, replacement), decision).ok;
  for (const key of ['parent', 'child', 'pr366', 'pr379', 'web_authority']) {
    assert.equal(evidence.pagination[key].transport_mode, 'DIRECT', key);
    assert.equal(evidence.pagination[key].page_size, null, key);
    assert.equal(evidence.pagination[key].progression, null, key);
    assert.equal(evidence.pagination[key].terminal_state, null, key);
    assert.equal(evidence.pagination[key].provider_total_count, null, key);
    assert.deepEqual(evidence.pagination[key].server_total, { status: 'UNAVAILABLE', value: null }, key);
    assert.equal(mutate(['pagination', key, 'server_total'], { status: 'AVAILABLE', value: evidence.pagination[key].retrieved_count }), false, key);
    assert.equal(mutate(['pagination', key, 'provider_total_count'], evidence.pagination[key].retrieved_count), false, key);
  }
  for (const key of ['native_children', 'current_label', 'reviews', 'threads', 'review_thread_comments', 'comments', 'checks']) {
    assert.equal(evidence.pagination[key].transport_mode, 'LINK', key);
    assert.equal(evidence.pagination[key].page_size, 100, key);
    assert.equal(evidence.pagination[key].progression.style, 'LINK', key);
    assert.equal(evidence.pagination[key].terminal_state.has_next_page, false, key);
  }
  assert.deepEqual(evidence.pagination.checks.server_total, { status: 'AVAILABLE', value: 6 });
  assert.equal(evidence.pagination.checks.provider_total_count, 6);
  assert.equal(evidence.provider_evidence.check_runs.value, 6);
  assert.equal(mutate(['pagination', 'checks', 'server_total'], { status: 'UNAVAILABLE', value: null }), false);
  assert.equal(mutate(['provider_evidence', 'check_runs', 'value'], 7), false);
  assert.equal(mutate(['pagination', 'checks', 'provider_total_count'], 7), false);
  assert.equal(mutate(['pagination', 'comments', 'server_total'], { status: 'AVAILABLE', value: 6 }), false);
  assert.equal(mutate(['pagination', 'comments', 'provider_total_count'], 6), false);
  assert.equal(mutate(['pagination', 'parent', 'page_size'], 100), false);
  assert.equal(mutate(['pagination', 'parent', 'progression'], { style: 'LINK', pages: [{ page: 1, next_url: null }] }), false);
  assert.equal(mutate(['pagination', 'comments', 'page_size'], null), false);
  assert.equal(mutate(['pagination', 'comments', 'progression'], null), false);
  assert.equal(mutate(['pagination', 'comments', 'terminal_state'], null), false);
  const missingProviderTotal = clone(evidence);
  delete missingProviderTotal.provider_evidence.check_runs.value;
  assert.equal(programme.validateEvidence(withEvidenceDigest(missingProviderTotal), decision).ok, false);
});

test('PR movement, check/review/thread/comment movement, labels, and native relationship movement fail closed', () => {
  const evidence = makeEvidence();
  assert.equal(programme.validateEvidence(replaceEvidence(evidence, ['pr_379', 'head'], 'f'.repeat(40)), decision).code, 'RECOVERY_PR379_INVALID');
  const reviewMoved = clone(evidence);
  reviewMoved.pr_379.reviews[0].state = 'APPROVED';
  assert.equal(programme.validateEvidence(withEvidenceDigest(reviewMoved), decision).code, 'RECOVERY_PR379_REVIEW_MOVED');
  const commentMoved = clone(evidence);
  commentMoved.pr_379.comments[0].body = 'moved';
  assert.equal(programme.validateEvidence(withEvidenceDigest(commentMoved), decision).code, 'RECOVERY_PR379_COMMENT_MOVED');
  const checksMoved = clone(evidence);
  checksMoved.pr_379.checks[0].conclusion = 'failure';
  assert.equal(programme.validateEvidence(withEvidenceDigest(checksMoved), decision).code, 'RECOVERY_PR379_CHECK_MOVED');
  assert.equal(programme.validateEvidence(replaceEvidence(evidence, ['pr_379', 'threads'], [{ id: 'thread-1' }]), decision).code, 'RECOVERY_PR379_THREAD_MOVED');
  assert.equal(programme.validateEvidence(replaceEvidence(evidence, ['child', 'labels'], ['current', 'other']), decision).code, 'RECOVERY_CHILD_EVIDENCE_INVALID');
  assert.equal(programme.validateEvidence(replaceEvidence(evidence, ['child', 'native_parent'], 1), decision).code, 'RECOVERY_CHILD_EVIDENCE_INVALID');
  assert.equal(programme.validateEvidence(replaceEvidence(evidence, ['parent', 'native_children'], [358, 359]), decision).code, 'RECOVERY_PARENT_EVIDENCE_INVALID');
});

test('partial write classifications require the same preview, authority, operation, and receipt binding', () => {
  const before = makeEvidence();
  const first = programme.projectionBootstrapRecovery.preview({ decision, evidence: before });
  assert.equal(first.ok, true, first.code);
  const continuation = continuationFromPreview(first);
  const childWritten = makeEvidence('CHILD_WRITTEN_PARENT_STALE', continuation);
  const continuationPreview = programme.projectionBootstrapRecovery.preview({ decision, evidence: childWritten });
  assert.equal(continuationPreview.ok, true, continuationPreview.code);
  assert.equal(continuationPreview.operation_count, 1);
  assert.deepEqual(continuationPreview.operation_order, [240]);
  const badContinuation = clone(childWritten);
  badContinuation.continuation.preview_id = 'f'.repeat(64);
  assert.equal(programme.projectionBootstrapRecovery.preview({ decision, evidence: withEvidenceDigest(badContinuation) }).ok, false);
  const badChildOperationDigest = clone(childWritten);
  badChildOperationDigest.continuation.child_operation_digest = 'f'.repeat(64);
  assert.equal(programme.projectionBootstrapRecovery.preview({ decision, evidence: withEvidenceDigest(badChildOperationDigest) }).ok, false);
  const wrongCombo = clone(before);
  wrongCombo.child.raw_body = continuationPreview.target.bodies.child;
  wrongCombo.child.body_digest = digestText(wrongCombo.child.raw_body);
  wrongCombo.child.canonical_digest = continuationPreview.target.canonical_digest;
  const wrongParsed = programme.parseChildV5Body(wrongCombo.child.raw_body, { repository, parent_issue: 240 });
  wrongCombo.child.projection = wrongParsed.envelope;
  wrongCombo.child.prefix_digest = wrongParsed.prefix_digest;
  wrongCombo.child.suffix_digest = wrongParsed.suffix_digest;
  wrongCombo.snapshot = 'BEFORE_CHILD';
  assert.equal(programme.projectionBootstrapRecovery.preview({ decision, evidence: withEvidenceDigest(wrongCombo) }).ok, false);
});

test('observable both-target snapshot returns zero operations', () => {
  const before = makeEvidence();
  const first = programme.projectionBootstrapRecovery.preview({ decision, evidence: before });
  const bothTarget = makeEvidence('PARENT_AND_CHILD_TARGET_OBSERVED', continuationFromPreview(first));
  const classification = programme.classifyPartialState({
    parent_canonical_digest: bothTarget.parent.canonical_digest,
    child_canonical_digest: bothTarget.child.canonical_digest,
    source_canonical_digest: programme.SOURCE_CANONICAL_DIGEST,
    target_canonical_digest: bothTarget.parent.canonical_digest,
  });
  assert.equal(classification.ok, true, classification.code);
  assert.equal(classification.classification, 'PARENT_AND_CHILD_TARGET_OBSERVED');
  const reconciled = programme.projectionBootstrapRecovery.preview({ decision, evidence: bothTarget });
  assert.equal(reconciled.ok, true, reconciled.code);
  assert.equal(reconciled.code, 'PROGRAMME_ZERO_DELTA');
  assert.equal(reconciled.operation_count, 0);
  assert.deepEqual(reconciled.operations, []);
  assert.equal(reconciled.recovery_retired, true);
  assert.equal(reconciled.duplicate_write, false);
  assert.equal(reconciled.readback_required, true);
});

test('self-retirement rejects a target with an unrelated canonical change', () => {
  const before = makeEvidence();
  const first = programme.projectionBootstrapRecovery.preview({ decision, evidence: before });
  const target = makeEvidence('PARENT_AND_CHILD_TARGET_OBSERVED', continuationFromPreview(first));
  const changed = clone(target);
  changed.parent.state.children.find((item) => item.issue === 360).summary = 'unrelated movement';
  assert.equal(programme.validateEvidence(withEvidenceDigest(changed), decision).ok, false);
});

test('receipt subsystem remains unchanged and the binding is truthful idempotent semantics', () => {
  const receiptPath = path.join(__dirname, '..', 'scripts', 'toolkit-github-program-receipt.cjs');
  const before = digestText(fs.readFileSync(receiptPath, 'utf8'));
  const descriptor = programme.buildReceiptOperationDescriptor({
    issue: 359,
    authority_digest: decision.web_authority.digest,
    source_body_digest: programme.SOURCE_CHILD_BODY_DIGEST,
    source_revision: programme.SOURCE_CHILD_REVISION,
    target_body_digest: 'f'.repeat(64),
  });
  assert.doesNotThrow(() => receipt.validateOperationDescriptor(descriptor));
  assert.equal(descriptor.operation_kind, 'IDEMPOTENT_SET');
  assert.equal(descriptor.safety_class, 'IDEMPOTENT');
  assert.equal(descriptor.cas_digest, programme.digestValue({
    mode: programme.WRITE_SAFETY_MODE,
    authority_digest: decision.web_authority.digest,
    source_body_digest: programme.SOURCE_CHILD_BODY_DIGEST,
    source_revision: programme.SOURCE_CHILD_REVISION,
  }));
  assert.equal(descriptor.adapter_identity_digest, programme.digestValue({
    adapter: 'github-web-readonly-adapter',
    mode: programme.WRITE_SAFETY_MODE,
    provider_cas_claim: false,
  }));
  assert.equal(receipt.OPERATION_KINDS.includes('CONDITIONAL_PROVIDER_UPDATE'), true);
  assert.equal(receipt.SAFETY_CLASSES.includes('CAS'), true);
  assert.equal(programme.WRITE_SAFETY_MODE, 'WEB_EXCLUSIVE_SINGLE_WRITER_RECOVERY_WINDOW');
  assert.equal(digestText(fs.readFileSync(receiptPath, 'utf8')), before);
});

test('post-merge decision and evidence schemas are closed and runtime-valid', () => {
  const finalDecision = finalisationDecision();
  const evidence = makeFinalisationEvidence();
  assert.ok(finalDecision);
  assert.equal(publishedFinalisationDecisionSchema.additionalProperties, false);
  assert.equal(publishedFinalisationEvidenceSchema.additionalProperties, false);
  assert.equal(validateDecisionAgainstPublishedSchema(finalDecision, publishedFinalisationDecisionSchema), true, 'decision schema');
  assert.equal(validateDecisionAgainstPublishedSchema(evidence, publishedFinalisationEvidenceSchema), true, 'evidence schema');
  assert.equal(programme.postMergeEpochFinalisation.validateDecision(finalDecision).ok, true, 'decision runtime');
  const evidenceValidation = programme.postMergeEpochFinalisation.validateEvidence(evidence, finalDecision);
  assert.equal(evidenceValidation.ok, true, evidenceValidation.code);
});

test('published finalisation schema resolves local refs and closes child state', () => {
  const finalDecision = finalisationDecision();
  const evidence = makeFinalisationEvidence();
  const positive = makeFinalisationEvidence('PARENT_STAGE_A_OBSERVED');
  assert.equal(validateDecisionAgainstPublishedSchema(finalDecision, publishedFinalisationDecisionSchema), true);
  assert.equal(validateDecisionAgainstPublishedSchema(evidence, publishedFinalisationEvidenceSchema), true);

  const rejects = [
    ['parent state null', (value) => { value.parent.state = null; }],
    ['parent state malformed', (value) => { value.parent.state = {}; }],
    ['parent state hidden extra key', (value) => { value.parent.state = { ...value.parent.state, unexpected: true }; }],
    ['child state null', (value) => { value.child.state = null; }],
    ['child state malformed', (value) => { value.child.state = {}; }],
    ['child state desired key', (value) => { value.child.state.desired = true; }],
    ['child state provider client key', (value) => { value.child.state.provider_client = false; }],
    ['child state provider cas key', (value) => { value.child.state.provider_cas_claim = false; }],
    ['child state programme apply key', (value) => { value.child.state.programme_apply = false; }],
    ['parent provider revision sentinel', (value) => { value.parent.revision = 'OPEN'; }],
    ['previous source binding scalar', (value) => { value.transaction.previous_source_binding = 42; }],
    ['previous source binding malformed child', (value) => { value.transaction.previous_source_binding.child = null; }],
    ['previous source binding hidden extra key', (value) => { value.transaction.previous_source_binding.child.unexpected = true; }],
  ];
  for (const [name, mutate] of rejects) {
    const candidate = clone(name.startsWith('previous') ? positive : evidence);
    mutate(candidate);
    assert.equal(
      validateDecisionAgainstPublishedSchema(candidate, publishedFinalisationEvidenceSchema),
      false,
      name,
    );
  }
});

test('clean inter-epoch Stage A and Stage B targets validate with exact E3/E4 and registry semantics', () => {
  const fixtures = finalisationStateFixtures();
  const stageAChild = fixtures.stageA.children.find((item) => item.issue === 359);
  const stageBChild = fixtures.stageB.children.find((item) => item.issue === 359);
  assert.equal(programme.validateCanonicalStateV5(fixtures.sourceState).ok, true);
  assert.equal(programme.postMergeEpochFinalisation.buildStageATargetState(fixtures.sourceState).design_lock, programme.FINALISATION_LOCK);
  assert.equal(programme.validateCanonicalStateV5(fixtures.stageA).ok, true);
  assert.equal(programme.validateCanonicalStateV5(fixtures.stageB).ok, true);
  for (const child of [stageAChild, stageBChild]) {
    assert.equal(child.lifecycle, 'CURRENT');
    assert.deepEqual(child.finality, { state: 'UNMERGED', authority_ref: null });
    assert.deepEqual(child.holds, []);
    assert.deepEqual(child.epochs.filter((epoch) => epoch.id !== 'E4').map((epoch) => epoch.terminal_disposition), ['ACCEPTED', 'ACCEPTED', 'ACCEPTED']);
    assert.deepEqual(child.epochs.find((epoch) => epoch.id === 'E4'), { ...child.epochs.find((epoch) => epoch.id === 'E4'), terminal_disposition: null, evidence_ref: null });
    assert.equal(child.pr_registry.find((entry) => entry.pr === 380).status, 'ACCEPTED');
    assert.equal(child.pr_registry.find((entry) => entry.pr === 380).github_state, 'MERGED');
    assert.equal(child.pr_registry.find((entry) => entry.pr === 380).completes_child, false);
  }
  assert.equal(stageAChild.pr_registry.find((entry) => entry.pr === 379).github_state, 'OPEN');
  assert.equal(stageBChild.pr_registry.find((entry) => entry.pr === 379).github_state, 'CLOSED');
  assert.equal(stageAChild.pr_registry.some((entry) => [376, 377, 378].includes(entry.pr)), false);
});

test('finalisation recognises the exact operation order and all bounded normal checkpoints', () => {
  const finalDecision = finalisationDecision();
  const checkpoints = [
    'BEFORE_STAGE_A',
    'CHILD_STAGE_A_OBSERVED',
    'PARENT_STAGE_A_OBSERVED',
    'PR379_CLOSED_STAGE_A',
    'CHILD_STAGE_B_OBSERVED',
    'FINAL_TARGET_OBSERVED',
  ];
  const expectedOrders = [
    ['CHILD_STAGE_A', 'PARENT_STAGE_A', 'PR379_CLOSE', 'CHILD_STAGE_B', 'PARENT_STAGE_B'],
    ['PARENT_STAGE_A', 'PR379_CLOSE', 'CHILD_STAGE_B', 'PARENT_STAGE_B'],
    ['PR379_CLOSE', 'CHILD_STAGE_B', 'PARENT_STAGE_B'],
    ['CHILD_STAGE_B', 'PARENT_STAGE_B'],
    ['PARENT_STAGE_B'],
    [],
  ];
  checkpoints.forEach((checkpoint, index) => {
    const evidence = makeFinalisationEvidence(checkpoint);
    const validation = programme.postMergeEpochFinalisation.validateEvidence(evidence, finalDecision);
    assert.equal(validation.ok, true, `${checkpoint}: ${validation.code}`);
    const classification = programme.postMergeEpochFinalisation.classifyCheckpoint({ decision: finalDecision, evidence });
    assert.equal(classification.ok, true, `${checkpoint}: ${classification.code}`);
    assert.equal(classification.checkpoint, checkpoint);
    const preview = programme.postMergeEpochFinalisation.preview({ decision: finalDecision, evidence });
    assert.equal(preview.ok, true, `${checkpoint}: ${preview.code}`);
    assert.deepEqual(preview.operation_order, expectedOrders[index]);
    assert.equal(preview.operation_count, expectedOrders[index].length);
    assert.equal(preview.programme_apply, false);
    assert.equal(preview.e4_auto_launch, false);
    assert.equal(preview.provider_cas_claim, false);
    assert.equal(preview.duplicate_write, false);
  });
});

test('Repair 2 rejects all ten unsafe exact-checkpoint acceptances', () => {
  const cases = [
    {
      name: 'child Stage A plus unauthorised prefix',
      checkpoint: 'CHILD_STAGE_A_OBSERVED',
      mutate: (value) => mutateFinalisationBody(value, 'child', (body) => 'unauthorised-prefix\n' + body),
    },
    {
      name: 'parent Stage A plus unauthorised prefix',
      checkpoint: 'PARENT_STAGE_A_OBSERVED',
      mutate: (value) => mutateFinalisationBody(value, 'parent', (body) => 'unauthorised-prefix\n' + body),
    },
    {
      name: '#379 close plus changed body digest',
      checkpoint: 'PR379_CLOSED_STAGE_A',
      mutate: (value) => {
        const next = clone(value);
        next.pr_379.body_digest = digestText('unauthorised #379 body');
        return recomputeFinalisationEvidence(next);
      },
    },
    {
      name: 'child Stage B plus unauthorised body movement',
      checkpoint: 'CHILD_STAGE_B_OBSERVED',
      mutate: (value) => mutateFinalisationBody(
        value,
        'child',
        (body) => body.replace(programme.MANAGED_MARKERS.child.end, '\nunauthorised-body-movement\n' + programme.MANAGED_MARKERS.child.end),
      ),
    },
    {
      name: 'final parent target plus unauthorised body movement',
      checkpoint: 'FINAL_TARGET_OBSERVED',
      mutate: (value) => mutateFinalisationBody(
        value,
        'parent',
        (body) => body.replace(programme.MANAGED_MARKERS.parent.end, '\nunauthorised-body-movement\n' + programme.MANAGED_MARKERS.parent.end),
      ),
    },
  ];
  for (const checkpoint of [
    'CHILD_STAGE_A_OBSERVED',
    'PARENT_STAGE_A_OBSERVED',
    'PR379_CLOSED_STAGE_A',
    'CHILD_STAGE_B_OBSERVED',
    'FINAL_TARGET_OBSERVED',
  ]) {
    cases.push({
      name: `#380 rebased revision/body/facts at ${checkpoint}`,
      checkpoint,
      mutate: rebaseFinalisationPr380,
    });
  }
  assert.equal(cases.length, 10);
  for (const item of cases) {
    const evidence = item.mutate(makeFinalisationEvidence(item.checkpoint));
    assert.equal(
      programme.postMergeEpochFinalisation.validateEvidence(evidence, finalisationDecision()).ok,
      false,
      item.name,
    );
  }
});

test('Repair 2 anchors immutable #380 against the original decision at every checkpoint', () => {
  const checkpoints = [
    'BEFORE_STAGE_A',
    'CHILD_STAGE_A_OBSERVED',
    'PARENT_STAGE_A_OBSERVED',
    'PR379_CLOSED_STAGE_A',
    'CHILD_STAGE_B_OBSERVED',
    'FINAL_TARGET_OBSERVED',
  ];
  for (const checkpoint of checkpoints) {
    const attacked = rebaseFinalisationPr380RevisionBody(makeFinalisationEvidence(checkpoint));
    const validation = programme.postMergeEpochFinalisation.validateEvidence(attacked, finalisationDecision());
    assert.equal(validation.ok, false, `${checkpoint}: #380 revision/body rebase`);
    assert.ok(
      ['FINALISATION_SOURCE_BINDING_INVALID', 'FINALISATION_EXACT_CHECKPOINT_PROVIDER_INVALID'].includes(validation.code),
      `${checkpoint}: anchored failure (${validation.code})`,
    );
  }
});

test('Repair 2 rejects rebased immutable fields in every prior checkpoint binding', () => {
  const cases = [
    ['CHILD_STAGE_A_OBSERVED', 'child', { prefix_digest: 'f'.repeat(64) }],
    ['PARENT_STAGE_A_OBSERVED', 'parent', { suffix_digest: 'e'.repeat(64) }],
    ['PR379_CLOSED_STAGE_A', 'pr_379', { body_digest: digestText('rebased prior #379 body') }],
    ['CHILD_STAGE_B_OBSERVED', 'child', { prefix_digest: 'd'.repeat(64) }],
    ['FINAL_TARGET_OBSERVED', 'parent', { suffix_digest: 'c'.repeat(64) }],
  ];
  for (const [checkpoint, resource, changes] of cases) {
    const attacked = rebasePreviousBinding(makeFinalisationEvidence(checkpoint), resource, changes);
    const validation = programme.postMergeEpochFinalisation.validateEvidence(attacked, finalisationDecision());
    assert.equal(validation.ok, false, `${checkpoint}: ${resource} prior binding rebase`);
    assert.equal(validation.code, 'FINALISATION_PREVIOUS_SOURCE_BINDING_NOT_ANCHORED', `${checkpoint}: anchored prior failure`);
  }
});

test('Repair 2 rejects unexpected resource movement, stale or fabricated prior state, and malformed revisions', () => {
  const finalDecision = finalisationDecision();
  const unexpected = [
    ['#240-only unexpected revision', rebindFinalisationProviderRevision(makeFinalisationEvidence('CHILD_STAGE_A_OBSERVED'), 'parent', 'synthetic:parent:unexpected')],
    ['#359-only unexpected revision', rebindFinalisationProviderRevision(makeFinalisationEvidence('PARENT_STAGE_A_OBSERVED'), 'child', 'synthetic:child:unexpected')],
    ['#379-only unexpected revision before close', rebindFinalisationProviderRevision(makeFinalisationEvidence('PARENT_STAGE_A_OBSERVED'), 'pr_379', 'synthetic:pr379:unexpected')],
    ['#380-only unexpected revision', rebaseFinalisationPr380RevisionBody(makeFinalisationEvidence('PARENT_STAGE_A_OBSERVED'))],
  ];
  const twoResource = rebindFinalisationProviderRevision(
    rebindFinalisationProviderRevision(makeFinalisationEvidence('CHILD_STAGE_A_OBSERVED'), 'parent', 'synthetic:parent:unexpected'),
    'pr_379',
    'synthetic:pr379:unexpected',
  );
  unexpected.push(['two-resource unexpected movement', twoResource]);
  for (const [name, evidence] of unexpected) {
    assert.equal(programme.postMergeEpochFinalisation.validateEvidence(evidence, finalDecision).ok, false, name);
  }

  const stale = makeFinalisationEvidence('PARENT_STAGE_A_OBSERVED');
  stale.transaction.previous_source_binding = clone(makeFinalisationEvidence('BEFORE_STAGE_A').source_binding);
  assert.equal(programme.postMergeEpochFinalisation.validateEvidence(withEvidenceDigest(stale), finalDecision).ok, false, 'stale prior binding');

  const fabricated = rebasePreviousBinding(
    makeFinalisationEvidence('CHILD_STAGE_A_OBSERVED'),
    'child',
    { canonical_digest: 'b'.repeat(64) },
  );
  assert.equal(programme.postMergeEpochFinalisation.validateEvidence(fabricated, finalDecision).ok, false, 'fabricated prior binding');

  const wrongCheckpoint = makeFinalisationEvidence('CHILD_STAGE_A_OBSERVED');
  wrongCheckpoint.transaction.checkpoint = 'PARENT_STAGE_A_OBSERVED';
  assert.equal(programme.postMergeEpochFinalisation.validateEvidence(withEvidenceDigest(wrongCheckpoint), finalDecision).ok, false, 'wrong checkpoint');

  const malformedRevision = makeFinalisationEvidence('BEFORE_STAGE_A');
  malformedRevision.parent.revision = 'OPEN';
  malformedRevision.source_binding.parent.revision = 'OPEN';
  assert.equal(programme.postMergeEpochFinalisation.validateEvidence(recomputeFinalisationEvidence(malformedRevision), finalDecision).ok, false, 'malformed revision');

  const changedFactsDigest = makeFinalisationEvidence('BEFORE_STAGE_A');
  changedFactsDigest.pr_380.facts.accepted_evidence_ref = 'changed';
  assert.equal(programme.postMergeEpochFinalisation.validateEvidence(withEvidenceDigest(changedFactsDigest), finalDecision).ok, false, 'changed facts digest');

  const changedSuffix = mutateFinalisationBody(
    makeFinalisationEvidence('CHILD_STAGE_A_OBSERVED'),
    'child',
    (body) => body + '\nunauthorised suffix',
  );
  assert.equal(programme.postMergeEpochFinalisation.validateEvidence(changedSuffix, finalDecision).ok, false, 'changed suffix');
});

test('Repair 1 binds arbitrary execution-time provider revisions and rejects revision movement', () => {
  const revisions = {
    parent: 'github:issue:240:revision:2026-09-09T14:00:01Z',
    child: 'github:issue:359:revision:2026-09-09T14:00:02Z',
    pr379: 'github:pull:379:revision:2026-09-09T14:00:03Z',
    pr380: 'github:pull:380:revision:2026-09-09T14:00:04Z',
  };
  const finalDecision = finalisationDecisionWithRevisions(revisions).decision;
  const evidence = makeFinalisationEvidence('BEFORE_STAGE_A', { revision_overrides: revisions });
  assert.equal(programme.postMergeEpochFinalisation.validateDecision(finalDecision).ok, true);
  assert.equal(programme.postMergeEpochFinalisation.validateEvidence(evidence, finalDecision).ok, true);
  assert.equal(evidence.source_binding.parent.revision, revisions.parent);
  assert.equal(evidence.source_binding.child.revision, revisions.child);
  assert.equal(evidence.source_binding.pr_379.revision, revisions.pr379);
  assert.equal(evidence.source_binding.pr_380.revision, revisions.pr380);

  const source = fs.readFileSync(path.join(__dirname, '..', 'scripts', 'toolkit-github-program-state-v5.cjs'), 'utf8');
  assert.doesNotMatch(source, /FINALISATION_SOURCE_(PARENT|CHILD)_REVISION/);
  assert.doesNotMatch(source, /2026-09-08T11:(02:43|01:45)Z/);

  for (const resource of ['parent', 'child', 'pr_379', 'pr_380']) {
    const moved = rebindFinalisationProviderRevision(evidence, resource, `${resource}:unexpected:movement`);
    assert.equal(
      programme.postMergeEpochFinalisation.validateEvidence(moved, finalDecision).ok,
      false,
      `${resource}: unexpected movement must fail closed`,
    );
  }
});

test('Repair 1 requires #379/#380 provider revisions and keeps provider state separate from revision', () => {
  const finalDecision = finalisationDecision();
  for (const resource of ['pr_379', 'pr_380']) {
    const missing = makeFinalisationEvidence();
    delete missing[resource].revision;
    assert.equal(
      programme.postMergeEpochFinalisation.validateEvidence(withEvidenceDigest(missing), finalDecision).ok,
      false,
      `${resource}: provider revision is mandatory`,
    );
  }

  const evidence = makeFinalisationEvidence('PARENT_STAGE_A_OBSERVED');
  const preview = programme.postMergeEpochFinalisation.preview({ decision: finalDecision, evidence });
  assert.equal(preview.ok, true, preview.code);
  const close = preview.operations.find((operation) => operation.operation_id === 'PR379_CLOSE');
  assert.ok(close);
  assert.equal(close.source_revision, evidence.pr_379.revision);
  assert.notEqual(close.source_revision, 'OPEN');
  assert.notEqual(close.source_revision, 'CLOSED');
  assert.equal(close.expected_provider_state, 'OPEN');
  assert.equal(close.target_provider_state, 'CLOSED');
  assert.equal(close.source_body_digest, evidence.pr_379.body_digest);
});

test('Repair 1 accepts only the expected single-resource rebind at every normal checkpoint', () => {
  const finalDecision = finalisationDecision();
  const checkpoints = [
    'CHILD_STAGE_A_OBSERVED',
    'PARENT_STAGE_A_OBSERVED',
    'PR379_CLOSED_STAGE_A',
    'CHILD_STAGE_B_OBSERVED',
    'FINAL_TARGET_OBSERVED',
  ];
  const expectedResource = {
    CHILD_STAGE_A_OBSERVED: 'child',
    PARENT_STAGE_A_OBSERVED: 'parent',
    PR379_CLOSED_STAGE_A: 'pr_379',
    CHILD_STAGE_B_OBSERVED: 'child',
    FINAL_TARGET_OBSERVED: 'parent',
  };
  for (const checkpoint of checkpoints) {
    const evidence = makeFinalisationEvidence(checkpoint);
    const validation = programme.postMergeEpochFinalisation.validateEvidence(evidence, finalDecision);
    assert.equal(validation.ok, true, `${checkpoint}: ${validation.code}`);
    const previous = makeFinalisationEvidence(FINALISATION_NORMAL_SPECS[checkpoint].previous);
    for (const resource of ['parent', 'child', 'pr_379', 'pr_380']) {
      if (resource === expectedResource[checkpoint]) {
        assert.notDeepEqual(evidence.source_binding[resource], previous.source_binding[resource], `${checkpoint}: expected movement`);
      } else {
        assert.deepEqual(evidence.source_binding[resource], previous.source_binding[resource], `${checkpoint}: unexpected movement`);
      }
    }
  }
  const mixed = rebindFinalisationProviderRevision(makeFinalisationEvidence('CHILD_STAGE_A_OBSERVED'), 'parent', 'unexpected:parent:movement');
  assert.equal(programme.postMergeEpochFinalisation.validateEvidence(mixed, finalDecision).ok, false);
});

test('acknowledgement-loss checkpoints require a fresh rebind and never blindly repeat a write', () => {
  const finalDecision = finalisationDecision();
  for (const checkpoint of programme.FINALISATION_ACK_LOSS_CHECKPOINTS) {
    const evidence = makeFinalisationEvidence(checkpoint);
    const validation = programme.postMergeEpochFinalisation.validateEvidence(evidence, finalDecision);
    assert.equal(validation.ok, true, `${checkpoint}: ${validation.code}`);
    const preview = programme.postMergeEpochFinalisation.preview({ decision: finalDecision, evidence });
    assert.equal(preview.ok, true, `${checkpoint}: ${preview.code}`);
    assert.equal(preview.acknowledgement_loss, true);
    assert.deepEqual(preview.operations, []);
    assert.equal(preview.status, 'REQUIRES_FRESH_COMPLETE_REBIND');
    assert.equal(preview.fresh_complete_rebind_required, true);
    assert.equal(preview.duplicate_write, false);
  }
});

test('Repair 1 acknowledgement loss recovers through a fresh normal checkpoint for all five operations', () => {
  const finalDecision = finalisationDecision();
  const operationIds = (checkpoint) => {
    const completed = FINALISATION_NORMAL_SPECS[checkpoint].completed;
    return programme.FINALISATION_OPERATION_ORDER
      .filter((operation) => !completed.includes(operation.order))
      .map((operation) => operation.operation_id);
  };
  for (const checkpoint of programme.FINALISATION_ACK_LOSS_CHECKPOINTS) {
    const spec = FINALISATION_ACK_SPECS[checkpoint];
    const lost = makeFinalisationEvidence(checkpoint, { ack_effect: false });
    const lostValidation = programme.postMergeEpochFinalisation.validateEvidence(lost, finalDecision);
    assert.equal(lostValidation.ok, true, `${checkpoint}: lost acknowledgement validation`);
    const lostPreview = programme.postMergeEpochFinalisation.preview({ decision: finalDecision, evidence: lost });
    assert.equal(lostPreview.ok, true, `${checkpoint}: lost acknowledgement preview`);
    assert.deepEqual(lostPreview.operations, []);
    assert.equal(lostPreview.status, 'REQUIRES_FRESH_COMPLETE_REBIND');

    const noEffect = makeFinalisationEvidence(spec.pre);
    const noEffectClassification = programme.postMergeEpochFinalisation.classifyCheckpoint({ decision: finalDecision, evidence: noEffect });
    assert.equal(noEffectClassification.ok, true, `${checkpoint}: no-effect classification`);
    assert.equal(noEffectClassification.checkpoint, spec.pre);
    const noEffectPreview = programme.postMergeEpochFinalisation.preview({ decision: finalDecision, evidence: noEffect });
    assert.equal(noEffectPreview.ok, true, `${checkpoint}: no-effect continuation`);
    assert.deepEqual(noEffectPreview.operation_order, operationIds(spec.pre));

    const effect = makeFinalisationEvidence(spec.base);
    const effectClassification = programme.postMergeEpochFinalisation.classifyCheckpoint({ decision: finalDecision, evidence: effect });
    assert.equal(effectClassification.ok, true, `${checkpoint}: effect classification`);
    assert.equal(effectClassification.checkpoint, spec.base);
    const effectPreview = programme.postMergeEpochFinalisation.preview({ decision: finalDecision, evidence: effect });
    assert.equal(effectPreview.ok, true, `${checkpoint}: effect continuation`);
    assert.deepEqual(effectPreview.operation_order, operationIds(spec.base));
  }
});

test('already-valid Stage A after #379 closure continues with Stage B only, and final rerun is exact zero delta', () => {
  const finalDecision = finalisationDecision();
  const closed = makeFinalisationEvidence('PR379_CLOSED_STAGE_A');
  const continuation = programme.postMergeEpochFinalisation.preview({ decision: finalDecision, evidence: closed });
  assert.equal(continuation.ok, true, continuation.code);
  assert.deepEqual(continuation.operation_order, ['CHILD_STAGE_B', 'PARENT_STAGE_B']);
  const finalEvidence = makeFinalisationEvidence('FINAL_TARGET_OBSERVED');
  const first = programme.postMergeEpochFinalisation.preview({ decision: finalDecision, evidence: finalEvidence });
  const second = programme.postMergeEpochFinalisation.preview({ decision: clone(finalDecision), evidence: clone(finalEvidence) });
  assert.equal(first.ok, true, first.code);
  assert.equal(first.code, 'FINALISATION_ZERO_DELTA');
  assert.equal(first.status, 'ALREADY_FINAL');
  assert.deepEqual(first.operations, []);
  assert.equal(first.operation_count, 0);
  assert.equal(first.zero_delta, true);
  assert.deepEqual(second, first);
  assert.equal(programme.postMergeEpochFinalisation.validateReadback({ decision: finalDecision, evidence: finalEvidence }).ok, true);
});

test('finalisation rejects wrong merge/candidate/evidence/authority identities and unsafe controls', () => {
  const finalDecision = finalisationDecision();
  const valid = makeFinalisationEvidence();
  const mutate = (pathParts, replacement) => programme.postMergeEpochFinalisation.validateEvidence(
    replaceFinalisationEvidence(valid, pathParts, replacement), finalDecision,
  ).ok;
  assert.equal(mutate(['pr_380', 'candidate', 'head'], 'f'.repeat(40)), false);
  assert.equal(mutate(['pr_380', 'candidate', 'tree'], 'f'.repeat(40)), false);
  assert.equal(mutate(['pr_380', 'base_sha'], 'f'.repeat(40)), false);
  assert.equal(mutate(['pr_380', 'github_state'], 'OPEN'), false);
  assert.equal(mutate(['pr_380', 'merge_commit'], 'f'.repeat(40)), false);
  assert.equal(mutate(['pr_380', 'ordered_parents'], [programme.PR380_HEAD, programme.MAIN_SHA]), false);
  assert.equal(mutate(['merge_ancestry', 'merge_commit'], 'f'.repeat(40)), false);
  assert.equal(mutate(['merge_ancestry', 'ordered_parents'], [programme.PR380_HEAD, programme.MAIN_SHA]), false);
  assert.equal(mutate(['accepted_evidence'], valid.accepted_evidence.slice(0, 2)), false);
  assert.equal(mutate(['accepted_evidence', 0, 'body_digest'], 'f'.repeat(64)), false);
  assert.equal(mutate(['write_authority', 'body'], 'changed authority body'), false);
  assert.equal(mutate(['source_revisions', 'parent', 'revision'], 'stale-parent'), false);
  assert.equal(mutate(['source_revisions', 'child', 'revision'], 'stale-child'), false);
  assert.equal(mutate(['pagination', 'collections', 0, 'complete'], false), false);
  assert.equal(mutate(['provider_facts', 'provider_cas_claim'], true), false);
  assert.equal(mutate(['accepted_epoch_event', 'occurrence'], 2), false);
  assert.equal(mutate(['transaction', 'checkpoint'], 'UNKNOWN'), false);
  const programmeApply = clone(valid);
  programmeApply.programme_apply = true;
  assert.equal(programme.postMergeEpochFinalisation.validateEvidence(withEvidenceDigest(programmeApply), finalDecision).ok, false);
});

test('finalisation rejects stale or manufactured canonical states and parent-child divergence', () => {
  const finalDecision = finalisationDecision();
  const valid = makeFinalisationEvidence('PARENT_STAGE_A_OBSERVED');
  const staleState = clone(valid);
  staleState.parent.state.children.find((item) => item.issue === 359).lifecycle = 'QUEUED';
  assert.equal(programme.postMergeEpochFinalisation.validateEvidence(withEvidenceDigest(staleState), finalDecision).ok, false);
  const noRegistry = clone(valid);
  noRegistry.parent.state.children.find((item) => item.issue === 359).pr_registry = noRegistry.parent.state.children.find((item) => item.issue === 359).pr_registry.filter((entry) => entry.pr !== 380);
  assert.equal(programme.postMergeEpochFinalisation.validateEvidence(withEvidenceDigest(noRegistry), finalDecision).ok, false);
  const e4Active = clone(valid);
  e4Active.parent.state.children.find((item) => item.issue === 359).epochs.find((epoch) => epoch.id === 'E4').terminal_disposition = 'ACCEPTED';
  assert.equal(programme.postMergeEpochFinalisation.validateEvidence(withEvidenceDigest(e4Active), finalDecision).ok, false);
  const manufacturedLane = clone(valid);
  manufacturedLane.parent.state.active_lanes = [{ child_issue: 359 }];
  assert.equal(programme.postMergeEpochFinalisation.validateEvidence(withEvidenceDigest(manufacturedLane), finalDecision).ok, false);
  const divergence = clone(valid);
  divergence.child.canonical_digest = 'f'.repeat(64);
  assert.equal(programme.postMergeEpochFinalisation.validateEvidence(withEvidenceDigest(divergence), finalDecision).ok, false);
  const prematureClose = makeFinalisationEvidence('BEFORE_STAGE_A');
  prematureClose.pr_379.state = 'CLOSED';
  prematureClose.pr_379.github_state = 'CLOSED';
  prematureClose.pagination = programme.postMergeEpochFinalisation.buildPaginationEvidence(prematureClose);
  assert.equal(programme.postMergeEpochFinalisation.validateEvidence(withEvidenceDigest(prematureClose), finalDecision).ok, false);
});

test('renderers derive recovery and clean inter-epoch truth from one canonical state', () => {
  const fixtures = finalisationStateFixtures();
  const recoveryRendered = programme.renderProgrammeV5(fixtures.sourceState);
  const cleanRendered = programme.renderProgrammeV5(fixtures.stageA);
  const closedRendered = programme.renderProgrammeV5(fixtures.stageB);
  for (const rendered of [cleanRendered, closedRendered]) {
    assert.match(rendered.parent, /#380 \| ACCEPTED \| MERGED/);
    assert.match(rendered.child, /E3: ACCEPTED/);
    assert.match(rendered.child, /E4: PENDING/);
    assert.match(rendered.child, /Finality \| UNMERGED/);
    assert.match(rendered.child, /Normal active lanes: 0/);
    assert.match(rendered.child, /Active blocking recovery hold: NO/);
    assert.match(rendered.child, /G4 active: NO/);
    assert.match(rendered.child, /Wait for separate Web E4 authority/);
  }
  assert.match(cleanRendered.parent, /#379 \| RETIRED \| OPEN/);
  assert.match(closedRendered.parent, /#379 \| RETIRED \| CLOSED/);
  assert.match(recoveryRendered.child, /Active blocking recovery hold: YES/);
  assert.match(recoveryRendered.child, /E3: UNACCEPTED/);
  assert.match(recoveryRendered.child, /Maintain the blocking recovery hold/);
});

test('recovery API remains unchanged and the governance facade exposes the same finalisation primitive', () => {
  assert.equal(programme.projectionBootstrapRecovery.validateDecision(decision).ok, true);
  assert.equal(governance.programmeV5.postMergeEpochFinalisation, programme.postMergeEpochFinalisation);
  const runtime = governance.createRuntime({ repository });
  assert.equal(runtime.programmeV5.postMergeEpochFinalisation, programme.postMergeEpochFinalisation);
  assert.equal(programme.programmeV5.postMergeEpochFinalisation, programme.postMergeEpochFinalisation);
});

test('bootstrap proof uses workspace bytes and never requires a historical Git object', (t) => {
  const bootstrapPath = path.join(__dirname, '..', '..', '.github', 'ai-agent-toolkit-programme.json');
  if (!fs.existsSync(bootstrapPath)) {
    t.skip('bootstrap commit B is intentionally absent during source commit validation');
    return;
  }
  const bootstrap = JSON.parse(fs.readFileSync(bootstrapPath, 'utf8'));
  const contractPath = path.join(__dirname, '..', 'contracts', 'github-program-reconciler', 'programme-surface-contract-v5.json');
  const proof = programme.verifyBootstrapWorkspaceProof({
    bootstrap,
    contract_bytes: fs.readFileSync(contractPath, 'utf8'),
    workspace_revision: bootstrap.toolkit_contract.revision,
  });
  assert.equal(proof.ok, true, proof.code);
  assert.equal(proof.historical_git_object_required, false);
  assert.equal(proof.actual_workspace_bytes, true);
  const source = fs.readFileSync(path.join(__dirname, '..', 'scripts', 'toolkit-github-program-state-v5.cjs'), 'utf8');
  assert.doesNotMatch(source, /git show|git cat-file|fetch-depth|unshallow/);
});

test('accepted shared facade exposes v5 recovery without a second reconciler', () => {
  assert.equal(governance.programmeV5.projectionBootstrapRecovery, programme.projectionBootstrapRecovery);
  const runtime = governance.createRuntime({ repository });
  assert.equal(runtime.programmeV5.projectionBootstrapRecovery, programme.projectionBootstrapRecovery);
  assert.equal(programme.programmeV5.projectionBootstrapRecovery, programme.projectionBootstrapRecovery);
});
