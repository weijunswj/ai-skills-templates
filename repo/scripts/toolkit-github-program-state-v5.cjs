...
function readDurableReceiptInventory(store, events, activeChain, options = {}) {
  const claimed = receiptBoundEvents(events);
  const referencedIds = [];
  const seenIds = new Set();
  for (const event of claimed) {
    for (const receiptId of [event.receipt_id, ...(event.consumed_receipt_ids || [])]) {
      if (!receiptId) continue;
      if (!sha256(receiptId)) return fail('GPR_RECEIPT_ID_INVALID', { receipt_id: receiptId });
      if (!seenIds.has(receiptId)) {
        seenIds.add(receiptId);
        referencedIds.push(receiptId);
      }
    }
  }
  ...
  if (!store || typeof store.readReceiptById !== 'function' || typeof store.readReceiptChain !== 'function') {
    return fail('durable-receipt-store-required');
  }
  ...
  for (const receiptId of historicalIds) {
    let returned;
    try { returned = store.readReceiptById(receiptId); } catch (error) {
      const code = typeof error?.code === 'string' && /^GPR_[A-Z0-9_]+$/.test(error.code)
        ? error.code : 'run-receipt-readback-failed';
      return fail(code, { receipt_id: receiptId });
    }
    if (!isRecord(returned) || returned.receipt_id !== receiptId) return fail('receipt-lookup-binding-invalid', { receipt_id: receiptId });
    const checked = validateReceiptObject(returned, {
      repository: options.repository,
      parent_issue: options.parent_issue,
    });
    if (!checked.ok) return checked;
    const receipt = checked.receipt;
    if (receipt.receipt_id !== receiptId) return fail('receipt-lookup-binding-invalid', { receipt_id: receiptId });
    lookedUp.set(receiptId, receipt);
    const references = referencesByRun.get(receipt.run_id) || [];
    references.push(receiptId);
    referencesByRun.set(receipt.run_id, references);
  }

  const historicalReceipts = [];
  for (const [runId, references] of referencesByRun) {
    let chain;
    try { chain = store.readReceiptChain(runId); } catch (error) {
      if (error?.code === 'GPR_RUN_NOT_STARTED' || error?.code === 'GPR_RECEIPT_NOT_FOUND') {
        return fail('receipt-not-persisted', { run_id: runId });
      }
      const code = typeof error?.code === 'string' && /^GPR_[A-Z0-9_]+$/.test(error.code)
        ? error.code : 'run-receipt-readback-failed';
      return fail(code, { run_id: runId });
    }
    if (!Array.isArray(chain) || chain.length === 0) return fail('receipt-not-persisted', { run_id: runId });
    ...
    if (!checked.ok) return checked;
    const chainById = new Map(checked.receipts.map((receipt) => [receipt.receipt_id, receipt]));
    for (const receiptId of references) {
      const member = chainById.get(receiptId);
      if (!member) return fail('receipt-not-persisted', { receipt_id: receiptId, run_id: runId });
      if (!same(member, lookedUp.get(receiptId))) return fail('receipt-lookup-binding-invalid', { receipt_id: receiptId, run_id: runId });
    }
...
function createMemoryDurableStore(initial = {}) {
  ...
  return Object.freeze({
    ...
    readReceiptChain(runId) {
      return receiptList.filter((entry) => entry.run_id === runId).map(clone);
    },
    readReceiptById(receiptId) {
      if (arguments.length !== 1 || !sha256(receiptId)) {
        const error = new Error('GPR_RECEIPT_ID_INVALID');
        error.code = 'GPR_RECEIPT_ID_INVALID';
        throw error;
      }
      const receipt = receiptList.find((entry) => entry.receipt_id === receiptId);
      if (!receipt) {
        const error = new Error('GPR_RECEIPT_NOT_FOUND');
        error.code = 'GPR_RECEIPT_NOT_FOUND';
        throw error;
      }
...
function createProgrammeRuntimeV5(options = {}) {
  ...
  function apply(input = {}) {
    ...
    const readInventory = (snapshot, activeChain) => readDurableReceiptInventory(store, [
