    ...
    let out: Vec<Value> = rows
        ...
        .map(|row| {
            ...
            match row["mode"].as_str().unwrap() {
                ...
                "holder" => {
                    let decoded = serde_json::from_slice::<HolderAttestation>(raw);
                    json!({"valid":decoded.is_ok_and(|a| a.validate().is_ok() && verify_holder_attestation(&a, &HolderKey::from_bytes([11; 32])).is_ok())})
                },
                "request" => match decode_request_frame(&encode_frame(raw).unwrap()) {
                    Ok(_) => json!({"valid":true}),
                    Err(e) => json!({"valid":false,"request_id":e.request_id(),"error_code":e.code(),"error_kind":format!("{:?}",e.kind)}),
                },
                "response" => {
